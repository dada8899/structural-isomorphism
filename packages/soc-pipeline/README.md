...
***REMOVED*** 1. Load your event sizes (e.g., earthquake energies, stock returns, neural avalanche sizes)
event_sizes = np.loadtxt("my_events.txt")  ***REMOVED*** any 1-D positive array

***REMOVED*** 2. Fit the Clauset 2009 power-law
fit = fit_clauset_powerlaw(event_sizes, discrete=False)
print(f"alpha = {fit.alpha:.2f}  xmin = {fit.xmin:.2g}  n_tail = {fit.n_tail}")

***REMOVED*** 3. Bootstrap a 95% CI
ci = bootstrap_ci(event_sizes, n_boot=200)
print(f"CI = [{ci.ci_low:.2f}, {ci.ci_high:.2f}]")

***REMOVED*** 4. Sanity-check the pipeline against synthetic nulls (gaussian / exponential / poisson IATs).
***REMOVED***    A healthy pipeline rejects power-law on ALL THREE.
nulls = synthetic_null()
for name, case in nulls.items():
    print(f"{name}: correctly_rejected = {case.correctly_rejected}")

***REMOVED*** 5. Render verdict
...
***REMOVED******REMOVED*** Reproducibility notebooks
