...
import soc_pipeline  ***REMOVED*** noqa: F401 — side-effect registers accessor
...
def test_int_dtype_accepted():
    ...
    ***REMOVED*** Should NOT raise on accessor construction
    _ = s.soc
    ***REMOVED*** Will likely error on fit because xmin too small etc; we just check no
    ***REMOVED*** type-error at the dtype check.
...
def test_fit_alpha_returns_float_near_truth():
    ...
    ***REMOVED*** Generous band (small-sample fluctuation + Clauset MLE bias)
...
def test_validate_with_matching_band_passes():
    ...
    assert v.alpha_ci_low <= v.alpha <= v.alpha_ci_high + 0.05  ***REMOVED*** CI sanity
...
def test_validate_with_non_matching_band_marks_out_of_band():
    ...
    ***REMOVED*** underlying verdict should be DEVIATING since alpha falls outside predicted
...
def test_label_falls_back_to_series_default_when_no_name():
    ...
    s = pd.Series(_pareto_series(n=2000, seed=2).values)  ***REMOVED*** name=None
    v = s.soc.validate(n_boot=0)
...
def test_different_seeds_give_different_bootstrap_ci():
    ...
    ***REMOVED*** Point estimate (alpha) is from the full sample so should be identical
    assert v1.alpha == pytest.approx(v2.alpha)
    ***REMOVED*** CI should differ because bootstrap RNG differs
    assert v1.alpha_ci_low != v2.alpha_ci_low or v1.alpha_ci_high != v2.alpha_ci_high


***REMOVED*** --------------------------------------------------------------- skip-bootstrap
...
def test_fit_alpha_does_not_run_bootstrap():
    ...
    ***REMOVED*** If this were running 200-boot it would be much slower; we just assert
    ***REMOVED*** the return type + non-None.
    alpha = s.soc.fit_alpha()
    assert isinstance(alpha, float)
...
def test_verdict_to_dict_is_json_friendly():
    ...
    ***REMOVED*** FitResult / BootstrapResult should NOT appear in to_dict
    assert "fit" not in d
