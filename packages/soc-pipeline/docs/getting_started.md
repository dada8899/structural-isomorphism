...
```bash
pip install soc-pipeline                ***REMOVED*** core
pip install "soc-pipeline[notebooks]"   ***REMOVED*** + jupyter + matplotlib
pip install "soc-pipeline[dev]"         ***REMOVED*** + tests + ruff + mypy
```

Requires Python >= 3.10. Pure-Python; no compiled extensions.

***REMOVED******REMOVED*** 5-minute quickstart
...
***REMOVED*** Your data: any 1-D positive array of event sizes
...
***REMOVED*** 3. Null check (pipeline must REJECT power-law on synthetic non-heavy-tail)
...
print(v)  ***REMOVED*** 'CONFIRMED' | 'CONFIRMED (literature band)' | 'DEVIATING'
```

***REMOVED******REMOVED*** Earthquake b-value

```python
from soc_pipeline import fit_b_value
mags = np.array([...])  ***REMOVED*** magnitudes from any earthquake catalog
bv = fit_b_value(mags, bootstrap=True)
print(f"b = {bv.b:.3f}  Mc = {bv.mc}  alpha_eq = {bv.alpha_equivalent:.3f}")
```

***REMOVED******REMOVED*** Omori aftershock decay

```python
from soc_pipeline import fit_omori_p, bin_and_omori_from_events
***REMOVED*** Stack mode: pass aftershock delays in seconds
om = fit_omori_p(aftershock_delays_sec)
print(f"p = {om.p:.3f}  R^2 = {om.R2:.3f}")

***REMOVED*** Stream mode: auto-detect main shocks
