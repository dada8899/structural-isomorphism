...
Fit a power-law to the tail of `x_data` using the Clauset-Shalizi-Newman
2009 method (KS minimisation on `xmin`, MLE on `alpha`).
...
| `rejects_power_law` | bool | True iff LR test rejected PL |
...
Non-parametric bootstrap: resample and refit alpha `n_boot` times. Returns
`BootstrapResult` with `alpha_mean`, `alpha_median`, `alpha_std`, `ci_low`,
`ci_high`, `n_boot_succeeded`.

## Null controls

### `synthetic_null(kind=None, n=20_000, seed=42)`

If `kind` is None, returns a `dict[str, NullCase]` over three synthetic
non-heavy-tail nulls: `gaussian_walk`, `exponential`, `poisson_iat`. Each
`NullCase` has `name`, `fit: FitResult`, `correctly_rejected: bool`.

A healthy pipeline rejects all three.
...
Stand-alone Vuong 1989 LR test: power-law vs `vs` (`"lognormal"` /
`"exponential"` / `"stretched_exponential"` / `"truncated_power_law"`).
...
Returns `CollapseResult.systems` mapping name → `SystemCurve` (`x_rescaled`,
`y_rescaled`, `raw_grid`, `raw_ccdf`, `alpha`, `s_star`, `n`).

## Omori

### `fit_omori_p(dts_sec, min_sec=300.0, max_sec=30*86400, n_bins=24, c_grid_days=(0.001, ..., 0.5)) -> OmoriResult`

Stack of aftershock delays after a main shock. Returns `p`, `p_sigma`,
`c_days`, `logK`, `R2`, `n_aftershocks_in_fit`, `n_bins_used`,
`t_range_days`.

### `bin_and_omori_from_events(event_times_sec, bin_seconds=60.0, sigma_k=3.0, window_bins=60) -> OmoriResult`

Auto-detect main shocks in an unlabelled event stream; run Omori-Utsu on
the resulting stack. Adds `n_main`, `mu`, `sigma`, `threshold` fields.

## b-value (Gutenberg-Richter)

### `fit_b_value(magnitudes, mc=None, bin_width=0.1, bootstrap=False, n_boot=500, seed=42) -> BValueResult`

Aki 1965 MLE. If `mc` is None, estimated by maximum-curvature. Returns `b`,
`sigma_b`, `mc`, `n_above_mc`, `alpha_equivalent`, optional `ci_low` /
`ci_high`.

### `b_to_clauset_alpha(b) -> float`

Map G-R `b` to equivalent Clauset `alpha` on energy via Hanks-Kanamori:
`alpha = 1 + b/1.5`.
...
Sweep bin sizes and fit alpha at each resolution. Returns `sweep` list,
`alpha_min/max/median/spread`, `is_stable`.
...
3-tier verdict: `'CONFIRMED'` / `'CONFIRMED (literature band)'` /
`'DEVIATING'` / `'INCONCLUSIVE'`.
