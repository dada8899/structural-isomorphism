...
```bash
pip install -e packages/cross-judge          ***REMOVED*** development
***REMOVED*** or, eventually:  pip install cross-judge   ***REMOVED*** not yet on PyPI
```
...
***REMOVED*** Three DeepSeek reviewers: 2 deterministic + 1 high-temperature dissenter
reviewers = [
    Reviewer(
...
def my_strategy(verdicts):
    ***REMOVED*** ... your logic ...
    return "KEEP", False  ***REMOVED*** (consensus, disagreement_bool)

panel = JudgePanel(reviewers=reviewers, strategy=my_strategy)
```

***REMOVED******REMOVED*** Vendors
...
***REMOVED******REMOVED*** Example: research-grade prompt
