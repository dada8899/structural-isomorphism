...
```bash
***REMOVED*** From source (pre-release)
...
```bash
pip install guarded-llm[anthropic]   ***REMOVED*** if you want the official anthropic SDK
pip install guarded-llm[openai]      ***REMOVED*** if you want the official openai SDK
pip install guarded-llm[dev]         ***REMOVED*** pytest, ruff, mypy
```
...
if result.ok:
    print(result.parsed)         ***REMOVED*** {"verdict": "REJECT", "confidence": 0.9}
    print(result.cost_usd)       ***REMOVED*** 0.0012
    print(result.attempts)       ***REMOVED*** 1
else:
...
***REMOVED******REMOVED*** Provider environment variables
