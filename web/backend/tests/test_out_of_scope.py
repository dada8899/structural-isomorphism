...
***REMOVED*** --------------------------------------------------------------------------
***REMOVED*** Layer B: LLM prompt branching
***REMOVED*** --------------------------------------------------------------------------
...
class PromptBranchingTests(unittest.TestCase):
    ...
    def test_low_relevance_prompt_contains_out_of_scope_instruction(self):
        ...
        ***REMOVED*** Out-of-scope branch must explicitly tell the model to decline.
        self.assertIn("超出", prompt)
        self.assertIn("覆盖范围", prompt)
        ***REMOVED*** Must tell the model NOT to use [n] citation markers.
...
***REMOVED*** --------------------------------------------------------------------------
***REMOVED*** Integration: full SSE stream emits out_of_scope flag
***REMOVED*** --------------------------------------------------------------------------
...
class OutOfScopeStreamTests(unittest.TestCase):
    ...
    def test_low_relevance_query_emits_out_of_scope_flag(self):
        ...
        ***REMOVED*** The guardrail must surface the gate decision so the frontend
        ***REMOVED*** can render an out-of-scope badge.
        self.assertTrue(answer_done.get("out_of_scope"))
        self.assertIn("scope_reason", answer_done)
        ***REMOVED*** The reason label encodes which gate tripped (top1_below_*, etc.)
    ...
    def test_high_relevance_query_does_not_emit_flag(self):
        ...
        events = _parse_sse(chunks)
        answer_done = next(p for n, p in events if n == "answer_done")
        ***REMOVED*** Field either absent or False — both acceptable.
    ...
    def test_low_relevance_prompt_handed_to_llm(self):
        ...
        ***REMOVED*** Out-of-scope keywords MUST be in the actual delivered prompt.
        self.assertIn("超出", prompt)
        self.assertIn("覆盖范围", prompt)
