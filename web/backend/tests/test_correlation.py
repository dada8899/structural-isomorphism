...
from logging_config import REQUEST_ID_VAR, configure_logging, get_logger  ***REMOVED*** noqa: E402
from middleware.correlation import (  ***REMOVED*** noqa: E402
    REQUEST_ID_HEADER,
...
***REMOVED*** UUID4 hex (no dashes) = 32 hex chars; we generate via uuid.uuid4().hex.
...
def app(tmp_path):
    ...
    ***REMOVED*** Point logs at a tmp file so we don't litter the repo.
    ...
    @a.get("/echo")
    async def echo():
        ***REMOVED*** Capture the contextvar inside a handler so the test can prove
        ***REMOVED*** the middleware set it before our code ran.
...
def test_request_without_header_gets_generated_uuid(client, app):
    ...
    ***REMOVED*** The same id must have been visible to the handler.
...
def test_malformed_id_replaced_with_uuid(client):
    ...
    r = client.get(
        "/echo",
        headers={REQUEST_ID_HEADER: "bad id with spaces and !@***REMOVED*** chars"},
    )
    rid = r.headers.get(REQUEST_ID_HEADER)
...
def test_log_line_includes_request_id(client, tmp_path):
    ...
    ***REMOVED*** Reconfigure to ensure logs land in a known tmp file.
    log_file = tmp_path / "server.jsonl"
    configure_logging(log_file=log_file)
    ...
    ***REMOVED*** Read the rotating file and look for at least one line tagged with
    ***REMOVED*** the supplied request_id + our custom event name.
    ...
    ***REMOVED*** And the per-request http.request line that middleware emits.
