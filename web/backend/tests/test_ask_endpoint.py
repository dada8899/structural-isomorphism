...
class AskOrchestratorTests(unittest.TestCase):
    def test_full_stream_event_sequence(self):
        ...
        ***REMOVED*** Spec ordering: meta first, kb_cards second, done last.
        self.assertEqual(names[0], "meta")
        self.assertEqual(names[1], "kb_cards")
        self.assertEqual(names[-1], "done")

        ***REMOVED*** All required event types present.
        for required in (
            "meta",
            "kb_cards",
            "answer_chunk",
...
class TestAskStreamEndpoint:
    ...
    def test_rate_limit_anonymous_429_after_burst(self, ask_app):
        ...
        ***REMOVED*** The endpoint default is "5/minute" for anonymous traffic. Hammering
        ***REMOVED*** 6 requests from the same client IP must trip the limiter on the
        ***REMOVED*** 6th. Use stream() for the first 5 so we exhaust the SSE body and
        ***REMOVED*** don't leave dangling generators.
        with TestClient(ask_app) as client:
            for _ in range(5):
                with client.stream(
        ...
        ***REMOVED*** slowapi raises RateLimitExceeded → handler returns 429.
        ***REMOVED*** If slowapi isn't installed in the env, this test would be skipped
        ***REMOVED*** via the decorator no-op path; in our deps it IS installed.
