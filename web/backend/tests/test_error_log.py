...
from api import error_log as el  ***REMOVED*** noqa: E402
...
def client(tmp_path, monkeypatch):
    ...
    ***REMOVED*** Reset rate-limit buckets between tests so we get a fresh window.
    el._buckets.clear()
...
def test_valid_report_stored(client):
    ...
    ***REMOVED*** File should have one line.
    assert log_path.exists()
    rows = log_path.read_text().splitlines()
    assert len(rows) == 1
    record = json.loads(rows[0])
    assert record["message"].startswith("TypeError")
    ***REMOVED*** URL query string must be stripped server-side.
...
def test_rate_limit_kicks_in(client):
    ...
    ***REMOVED*** First 10 must be accepted.
    ...
    ***REMOVED*** 11th in the same window must be rejected as rate-limited.
    r = c.post("/api/errors", json=payload)
    assert r.status_code == 200
    body = r.json()
...
def test_rate_limit_per_session_independent(client):
    ...
    ***REMOVED*** Alpha is now capped; beta should still go through.
...
def test_rotation_at_10mb(client, monkeypatch):
    c, log_path = client
    ***REMOVED*** Shrink the rotation threshold so we don't actually write 10MB in test.
    monkeypatch.setattr(el, "MAX_LOG_BYTES", 2048)
    payload = _valid_payload(stack="x" * 400, sessionId="rot-test")
    ***REMOVED*** 10 records ≈ enough to exceed 2KB once.
    for i in range(el.RATE_LIMIT_MAX):
        r = c.post("/api/errors", json=payload)
        assert r.status_code == 200
    rotated = log_path.with_suffix(log_path.suffix + ".1")
    ***REMOVED*** Either active or rotated must exist; rotation only triggers when size
    ***REMOVED*** threshold is crossed *before* the next write.
    assert log_path.exists()
    ***REMOVED*** The rotation file is best-effort; assert no crash + at least one record.
    record = json.loads(log_path.read_text().splitlines()[-1])
    assert record["sessionId"] == "rot-test"
    ***REMOVED*** rotated file existence depends on cumulative byte size — assert it's
    ***REMOVED*** either absent (small payloads) or non-empty.
    if rotated.exists():
...
def test_anonymous_no_session_uses_ip_bucket(client):
    ...
    for _ in range(el.RATE_LIMIT_MAX):
        r = c.post("/api/errors", json=payload)
        assert r.json()["accepted"] is True
    ***REMOVED*** Same IP, no session → bucketed together → 11th rejected.
    r = c.post("/api/errors", json=payload)
