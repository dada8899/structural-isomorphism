...
from errors import BudgetExceeded  # noqa: E402
from services.cost_ledger import _DailyCallLedger  # noqa: E402
...
def test_under_cap_charges_succeed(ledger, monkeypatch):
    ...
    # 5 charges allowed (count 0..4 < cap).
    for _ in range(5):
...
def test_charge_over_cap_raises_budget_exceeded(ledger, monkeypatch):
    ...
    for _ in range(3):
        ledger.charge()
    # 4th charge is over cap.
    with pytest.raises(BudgetExceeded) as ei:
        ledger.charge(endpoint="/api/analyze/stream")
    # BudgetExceeded is an RFC 7807 ProblemDetail — status 429, friendly.
...
def test_default_cap_when_env_missing(ledger, monkeypatch):
    ...
    snap = ledger.snapshot()
    assert snap["cap"] == 500  # _DEFAULT_DAILY_CALL_CAP
...
def test_new_utc_day_resets_counter(ledger, monkeypatch):
    ...
    with pytest.raises(BudgetExceeded):
        ledger.charge()
    # Simulate a day boundary by rewinding the ledger's stored date.
    ledger._date = "2000-01-01"
    # Next charge sees a new day → counter rolls → succeeds.
    ledger.charge()
