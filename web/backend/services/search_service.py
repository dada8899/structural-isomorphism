...
class SearchService:
    ...
    def __init__(
        ...
        ***REMOVED*** id -> list index, built at load time so mapping / analyze endpoints
        ***REMOVED*** can skip O(N) scans on every request.
        self.idx_by_id: Dict[str, int] = {}
        self._load_kb()

        ***REMOVED*** Bind the instance method to the query embedding cache. We use a
        ***REMOVED*** per-instance lru_cache (attached in __init__) so that replacing the
        ***REMOVED*** service (during reload) doesn't retain stale embeddings.
        self._encode_query_cached = lru_cache(maxsize=1024)(self._encode_query_uncached)

        ***REMOVED*** Try precomputed embeddings first (saves ~10 min on CPU-only machines)
        ...
        ***REMOVED*** Encode KB if no precomputed embeddings
    ...
    ***REMOVED*** encode_texts does a forward pass through the sentence-transformer model;
    ***REMOVED*** on CPU this is ~200-400ms. Caching by raw query string removes that cost
    ***REMOVED*** for repeated searches (e.g. examples endpoint + common user questions).
