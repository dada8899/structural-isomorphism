...
def tier_limit_decorator(default_anon: str = "10/minute"):
    ...
    ***REMOVED*** slowapi's dynamic-limit callable is invoked without the request
    ***REMOVED*** object (signature: `_spec()` or `_spec(key)` where key = remote-addr),
    ***REMOVED*** so we cannot resolve the tier inside the limit-provider. Tier-aware
    ***REMOVED*** auth still happens at the endpoint top via verify_api_token; here we
    ***REMOVED*** apply the anonymous baseline. A future enhancement could keep a
    ***REMOVED*** request-context contextvar to lift the tier into the limit provider.
