...
def tier_limit_decorator(default_anon: str = "10/minute"):
    ...
    ***REMOVED*** Local import to avoid a hard dep at module load — auth.py only needs
    ***REMOVED*** to be importable when this decorator actually fires.
    try:
    ...
    def _spec_for(request) -> str:
        ...
        ***REMOVED*** Invalid token → fall back to default_anon; the endpoint itself
        ***REMOVED*** is expected to raise 401, but if it doesn't we still rate-limit.
