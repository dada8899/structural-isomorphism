"""POST /api/stress-test — 结构压力测试（Session ***REMOVED***18, feature E）.
...
class StressTestRequest(BaseModel):
    ***REMOVED*** Bounds mirror stress_test_service.validate_claim — pydantic rejects the
    ***REMOVED*** obvious abuse early; validate_claim re-checks after .strip().
...
async def stress_test(request: Request, req: StressTestRequest):
    ...
    ***REMOVED*** No API key locally / in tests → be honest, don't fake an analysis.
