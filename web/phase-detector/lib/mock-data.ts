...
//
// 2026-05-23 PD-EWS: added mock EWS records (US + HK) so the new
// trajectory chart + leaderboard work in mock mode without a backend.

import type {
  Company, Stats, EwsCard, EwsResultFull, EwsMeta,
} from "./types";
...
// ---------------------------------------------------------------------------
// 2026-05-23 PD-EWS — Mock EWS payloads.
// Generated by run_ews_pipeline.py --demo against an 8-ticker US+HK sample
// (NVDA TSLA AAPL INTC 0700.HK 9988.HK 3690.HK 1810.HK). Bundled as JSON
// so the frontend renders meaningful charts in NEXT_PUBLIC_USE_MOCK=true
// without needing a backend.
// ---------------------------------------------------------------------------

import mockEwsRaw from "./mock-ews-data.json";

// The JSON import is typed loosely — cast at the boundary so the rest of the
// codebase sees the strict types.
const _mockEws = mockEwsRaw as {
  results: Record<string, EwsResultFull>;
  leaderboard: EwsCard[];
  meta: EwsMeta;
};

export const MOCK_EWS_DETAIL: Record<string, EwsResultFull> = _mockEws.results;
export const MOCK_EWS_LEADERBOARD: EwsCard[] = _mockEws.leaderboard;
export const MOCK_EWS_META: EwsMeta = _mockEws.meta;
