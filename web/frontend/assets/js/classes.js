...
function renderList(list) {
  ...
  if (!host) return;
  if (!list || !list.length) {
    host.innerHTML = `<p style="color:***REMOVED***777;padding:40px 0;text-align:center;">没有匹配的等价类。</p>`;
    return;
  }
...
async function init() {
  try {
    ...
    const host = document.getElementById("uc-list");
    if (host) {
      host.innerHTML = `<p style="color:***REMOVED***c44;padding:40px 0;text-align:center;">加载数据失败：${escapeHtml(e.message)}</p>`;
    }
  }
}
