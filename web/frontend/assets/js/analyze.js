...
function renderStreamError({ message, retryable }) {
  ...
  loading.innerHTML = `
    <h2 class="analyze-loading__title" style="color: var(--danger, ***REMOVED***dc2626)">生成失败</h2>
...
function buildBriefMarkdown() {
  ...
  const lines = [];
  lines.push(`***REMOVED*** Structural · 跨学科分析简报`);
  lines.push('');
  if (userQuery) {
  ...
  // Core insight
  if (struct.intuition) {
    lines.push(`***REMOVED******REMOVED*** 一句话核心`);
  ...
  if (insights.length > 0) {
    lines.push(`***REMOVED******REMOVED*** 三条最关键的洞察`);
  ...
  if (ifShort) {
    lines.push(`***REMOVED******REMOVED*** ⭐ 如果你只能做一件事`);
  ...
  if (actions.length > 0) {
    lines.push(`***REMOVED******REMOVED*** 本周行动清单`);
    actions.forEach((it) => {
      const rank = it.rank || '?';
  ...
  if (action.next_week_followup) {
    lines.push(`***REMOVED******REMOVED*** 下周回头看`);
  ...
  // Risks
  if (risks.length > 0) {
    lines.push(`***REMOVED******REMOVED*** 迁移风险`);
