"""Session ***REMOVED***9 W5-D — /company/[ticker] detail page audit.
...
W4-B (***REMOVED***113) wrapped CompanyCard in Link → /company/[ticker]. W5-D verifies
...
def _audit_detail(page: Page, ticker: str, viewport_name: str) -> dict[str, Any]:
    ...
    ***REMOVED*** W5-D: use domcontentloaded not networkidle — Plausible beacons + font
    ***REMOVED*** warming can keep the network active past the 30s default, even though
    ***REMOVED*** the page is fully usable. domcontentloaded fires once HTML is parsed,
    ***REMOVED*** then we explicitly wait for the H1 (post-fetchCompany hydration).
    page.goto(url, wait_until="domcontentloaded", timeout=30_000)

    ***REMOVED*** Wait for fetchCompany() to populate. Skeleton loader has
    ***REMOVED*** `animate-pulse` class; once gone, content is rendered.
    try:
        page.wait_for_selector("h1", timeout=15_000)
    except PWTimeout:
        pass  ***REMOVED*** capture screenshot anyway

    ***REMOVED*** Give React a beat to flush state after the API responds.
    time.sleep(1.5)
    ...
    ***REMOVED*** CPS badge — text "稳态远离" / "接近临界" / "已翻转" / etc.
    ...
    ***REMOVED*** Dynamics family label nearby — heuristic: contains common CN words
    ...
    ***REMOVED*** Primary indicators panel
    ...
    ***REMOVED*** Metadata toggle (or absent if no universality_class)
    ...
    ***REMOVED*** Top nav (cross-page navigation back)
    ...
    ***REMOVED*** Cross-site link to Structural beta
    asserts["structural_cross_link"] = page.locator("a[href*='beta.structural.bytedance.city']").count() > 0

    ***REMOVED*** Skeleton must not still be visible (means fetch failed or pending)
...
def test_company_detail_page(ticker: str, viewport_name: str, viewport: dict[str, int]) -> None:
    ...
    ***REMOVED*** Soft-assert: we record everything but only hard-fail on the cardinal
    ***REMOVED*** signals (page renders + tldr text appears). Other gaps go into the
    ***REMOVED*** audit doc as improvement TODOs, not test failures.
