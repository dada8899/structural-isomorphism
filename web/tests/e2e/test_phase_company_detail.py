"""Session ***REMOVED***10 W6-C — /company/[ticker] detail page polish e2e.
...
Run:
    cd /Users/dadamini/Projects/structural-isomorphism  ***REMOVED*** or worktree
...
***REMOVED*** CLS + LCP budgets per task acceptance.
CLS_BUDGET = 0.1
LCP_BUDGET_MS = 2500
...
***REMOVED*** W6-C: when running against prod that does NOT yet have these assertions
***REMOVED*** deployed, several "new behavior" asserts will soft-fail. Hard-fail set =
***REMOVED*** cardinal signals only (page renders + TL;DR text appears). Soft-fail set =
***REMOVED*** w6-c-specific asserts get recorded but don't block the suite — they're the
***REMOVED*** bar the deploy must clear to count as ship.
...
def _shot(page: Page, name: str) -> str:
    ...
    try:
        ...
        ***REMOVED*** Also drop a copy into docs/screenshots/w6c-* so the audit followup
        ***REMOVED*** doc can reference a stable location.
...
def _collect_perf(page: Page) -> dict[str, float | None]:
    ...
    try:
        ***REMOVED*** CLS via Layout Shift entries (cumulative since page load).
        cls = page.evaluate(
            """
            () => new Promise((resolve) => {
              let cls = 0;
        ...
        ***REMOVED*** LCP via LargestContentfulPaint buffered entries.
        lcp = page.evaluate(
            """
            () => new Promise((resolve) => {
              let lcp = null;
...
def _audit_detail(page: Page, ticker: str, viewport_name: str) -> dict[str, Any]:
    ...
    ***REMOVED*** Filter for app-origin errors only. 3rd-party analytics / fonts /
    ***REMOVED*** cross-origin resources that fail in some network environments are not
    ***REMOVED*** app bugs and should not break the suite.
    IGNORE_SUBSTRINGS = (
    ...
    def _on_console(msg) -> None:  ***REMOVED*** noqa: ANN001
        if msg.type != "error":
            return
        text = msg.text or ""
        loc = ""
    ...
    ***REMOVED*** Give React + fetchCompany a beat to flush + skeleton to clear.
    time.sleep(1.8)
    ...
    ***REMOVED*** CPS badge — preferred selector w6-c data-testid.
    ...
    if cps_badge.count() > 0:
        ...
        ***REMOVED*** legacy aria-label fallback
        legacy = page.locator(
    ...
    ***REMOVED*** Dynamics family label nearby
    ...
    ***REMOVED*** W6-C new: KeyContextPanel
    asserts["key_context_panel_present"] = (
        page.locator("[data-testid='key-context-panel']").count() > 0
    )

    ***REMOVED*** W6-C new: indicators panel uses CN labels, not raw `ar1_trend`
    ...
    if indicators_panel.count() > 0:
        ...
        ***REMOVED*** Confirm we do NOT show raw snake_case indicator keys.
    ...
    ***REMOVED*** W6-C: continuation nav with 返回公司表 + Structural cross-link.
    ...
    ***REMOVED*** W6-C: mobile hamburger present below sm:640px.
    ...
    if viewport_name == "mobile":
        ...
        asserts["mobile_nav_toggle_visible"] = None  ***REMOVED*** hidden by sm:hidden

    ***REMOVED*** Skeleton gone
    ...
    ***REMOVED*** Performance (best-effort)
...
def test_company_detail_page(
    ...
    ***REMOVED*** Hard fail: page renders + TL;DR appears (cardinal signal).
...
def teardown_module(module: Any) -> None:  ***REMOVED*** noqa: ARG001
