...
def load_spikes(nwb_path: Path):
    with h5py.File(nwb_path, "r") as f:
        ...
        ***REMOVED*** spike_times_index[i] is the cumulative end index for unit i
        idx = f["units/spike_times_index"][:]
        unit_ids = f["units/id"][:]
    print(f"  n_units: {len(unit_ids)}, n_spikes: {len(spike_times)}")
    ***REMOVED*** Reconstruct per-unit times for reference, but we mainly need the
    ***REMOVED*** pooled time series for avalanche analysis
...
def bin_and_detect_avalanches(pooled: np.ndarray, bin_width_s: float):
    ...
    ***REMOVED*** counts[j] = number of spikes in bin j
...
def main():
    ...
    ***REMOVED*** Compute average IEI
    iei = np.diff(pooled)
    ***REMOVED*** Drop zero-diff ties (simultaneous spikes across units)
    iei_pos = iei[iei > 0]
