...
Usage:
  ...
  ***REMOVED*** writes MANIFEST.json at the bundle root
"""
...
***REMOVED*** Heuristic: filename pattern -> license + schema reference
def classify(relpath: str) -> tuple[str, Optional[str]]:
    ...
    if p.endswith((".py", ".ipynb", ".toml", ".cff")) or p.endswith("makefile"):
        ...
        ***REMOVED*** schema YAMLs are MIT (machine-readable code); docs/README are CC-BY-4.0
    ...
    ***REMOVED*** Schema cross-references
    schema_ref: Optional[str] = None
    if p.endswith(".jsonl"):
        if "taxonomy" in p:
            schema_ref = "README.md***REMOVED***5-schemas-taxonomy-jsonl"
        elif "results" in p or "layer3" in p or "layer4" in p or "b3" in p:
            schema_ref = "README.md***REMOVED***5-schemas-results-jsonl"
        elif "registry" in p or "null" in p:
            schema_ref = "README.md***REMOVED***5-schemas-nulls-jsonl"
        else:
            schema_ref = "README.md***REMOVED***5-schemas-jsonl"
    elif p.endswith(".csv"):
        schema_ref = "README.md***REMOVED***5-schemas-csv"
    elif p.endswith(".yaml") and "/taxonomy/" in p:
        schema_ref = "README.md***REMOVED***5-schemas-class-yaml"
...
def iter_files(root: Path):
    for base, dirs, files in os.walk(root):
        ***REMOVED*** in-place prune of excluded dirs
...
def main():
    ...
    for fpath in iter_files(BUNDLE):
        ...
        ***REMOVED*** skip the manifest itself + this script's pycache
    ...
    ***REMOVED*** Repo provenance
    repo_root = BUNDLE.parents[2]  ***REMOVED*** dataset/v1/<bundle> -> repo root
    git_commit = detect_git_commit(repo_root) or os.environ.get("GIT_COMMIT_SHA", "unknown")

    ***REMOVED*** Sort entries deterministically
    ...
    ***REMOVED*** Compute aggregate sha256 over the file-entry blob (gives a single
    ***REMOVED*** "bundle hash" that downstream pins can lock against).
    ...
    ***REMOVED*** Idempotent write — sort_keys + indent
