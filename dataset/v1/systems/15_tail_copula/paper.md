***REMOVED*** Cross-Domain Tail Copula Test: Are Financial Extremes and Natural-Disaster Extremes Co-Occurring?
...
**Date.** 2026-05-13. Version: A2 Phase 6 (universality class ***REMOVED***6) preprint draft.
...
Layer 4 of the Structural Isomorphism project (V4) places, alongside the better-known SOC threshold-cascade class, a second proposed equivalence class *****REMOVED***6: tail copula** — the claim that heavy-tailed marginals from genuinely different domains can share *joint* extreme behaviour, captured by a non-zero upper tail dependence coefficient $\lambda_U$. The literature gives strong $\lambda_U \in [0.1, 0.4]$ for pairs *within* finance (equity indices, FX, credit) and for many *within*-climate pairs (precipitation, temperature). The strongest possible cross-domain version of class ***REMOVED***6 is that two domains whose only contact is the macro-economy — equity prices and natural-disaster damages — still exhibit $\lambda_U > 0$. We test this on 7,300 trading days (1996-2024) joining the S&P 500 daily absolute log returns to total NOAA Storm Events damage on the same calendar day. Pseudo-uniform marginals are obtained by empirical CDF rank transform; $\lambda_U$ is estimated three ways (empirical at quantiles $0.90, 0.95, 0.975, 0.99$; Gumbel-copula MLE; survival-Clayton MLE), with block-bootstrap 95% CIs and a 1000-replicate permutation null. The empirical $\lambda_U(0.95) = 0.052$ is indistinguishable from the independence null mean of $0.050$ ($p_{99}$ of null $= 0.077$); the Gumbel MLE drives $\theta \to 1$ (independence boundary, $\lambda_U \to 0$); survival Clayton drives $\theta \to 0$ ($\lambda_U \to 0$); and the conditional baseline lift $P(\text{extreme storm}|\text{extreme return})/P(\text{extreme storm}) = 1.04$ is essentially unity. **A within-domain positive control** — $|r_t|$ versus $|r_{t-1}|$ — recovers $\lambda_U(0.95) = 0.21$ and Gumbel $\lambda_U = 0.138$, well inside the published volatility-clustering range, so the methodology is not at fault. The verdict for the cross-domain version of universality class ***REMOVED***6 is **REJECTS** on this domain pair. We interpret this as a sharp narrowing of the class: tail copula universality holds *within* mechanism-coupled domains (within finance, within climate, within network cascades) but does not extend to two domains separated by a macroeconomic lag of weeks-to-quarters. This is itself a falsifiable, transferable prediction.

---

***REMOVED******REMOVED*** 1. Introduction

Empirical content of the Structural Isomorphism programme rests on members of a putative universality class behaving the same way under the same analysis. Phase 1 (SOC inverse-cubic / Omori) recovered the canonical SOC exponents on earthquakes and on the S&P 500 with no parameter retuning, supporting class ***REMOVED***1. Universality class ***REMOVED***6, "tail copula", is a different claim: not about the marginal exponent of a single series but about the **joint** behaviour of two series at the upper extreme.
...
4. A clean negative finding for cross-domain class ***REMOVED***6 with explicit boundary conditions.

***REMOVED******REMOVED*** 2. Data

***REMOVED******REMOVED******REMOVED*** 2.1 S&P 500 daily returns
...
***REMOVED******REMOVED******REMOVED*** 2.2 NOAA Storm Events 1996-2024
...
***REMOVED******REMOVED******REMOVED*** 2.3 Joint sample
...
***REMOVED******REMOVED******REMOVED*** 3.1 Marginal transform
...
***REMOVED******REMOVED******REMOVED*** 3.2 Three $\lambda_U$ estimators
...
***REMOVED******REMOVED******REMOVED*** 3.3 Inference: block bootstrap + permutation null
...
***REMOVED******REMOVED******REMOVED*** 3.4 Positive control

To rule out an estimator failure, we run the identical pipeline on $(|r_t|, |r_{t-1}|)$ within the financial series. Volatility clustering is one of the oldest and most robust stylised facts in finance; under the empirical class ***REMOVED***6 claim, $\lambda_U$ for $(|r_t|, |r_{t-1}|)$ should be solidly positive.

***REMOVED******REMOVED******REMOVED*** 3.5 Baseline lift
...
***REMOVED******REMOVED******REMOVED*** 4.1 Headline numbers
...
***REMOVED******REMOVED******REMOVED*** 4.2 Permutation null
...
***REMOVED******REMOVED******REMOVED*** 4.3 Baseline lift
...
***REMOVED******REMOVED******REMOVED*** 4.4 Positive control
...
***REMOVED******REMOVED******REMOVED*** 5.1 What the null does and does not say
...
***REMOVED******REMOVED******REMOVED*** 5.2 What this implies for class ***REMOVED***6

Universality class ***REMOVED***6 ("tail copula") as initially stated in Layer 4 conflated two distinct claims:

| Sub-class | Statement | Status |
|---|---|---|
| ***REMOVED***6a (mechanism-coupled) | Pairs sharing common drivers (within-finance / within-climate / within-network) have $\lambda_U \gg 0$. | Literature-supported; positive control here ($\lambda_U = 0.14$) consistent. |
| ***REMOVED***6b (cross-mechanism) | Pairs whose only contact is the macro-economy still have $\lambda_U > 0$. | **Rejected** on this domain pair at daily frequency. |

The clean separation is itself an empirical advance: it tells the project's Layer 2 community-detection step that **upper tail dependence is not a universal property of heavy-tailed bivariates** — it requires a shared mechanism. This narrows the predictive content of class ***REMOVED***6, which is the right direction for an empirical research programme.

***REMOVED******REMOVED******REMOVED*** 5.3 Relationship to the broader Structural Isomorphism programme

Class ***REMOVED***1 (SOC) recovers identical exponents on physics and finance because both members are individually generated by the same cascade mechanism. Class ***REMOVED***6's claim is structurally stronger — it would require a shared coupling between two independently-generated series. The data here say no such coupling exists between US weather and US equity prices at the daily scale. Class ***REMOVED***6 should therefore be re-stated as a **conditional** universality class (mechanism-coupled domains only), not a universal property of all heavy-tailed pairs.
...
```
v4/validation/tail-copula/fetch_data.py     ***REMOVED*** NOAA download + daily aggregation
v4/scripts/a2_copula_tail_dependence.py     ***REMOVED*** all estimators, bootstrap, null, control
```
...
```
v4/validation/tail-copula/storm_daily_damage.csv   ***REMOVED*** daily-aggregated storm damage
v4/validation/tail-copula/results.json             ***REMOVED*** all numerical outputs
v4/validation/tail-copula/paper.md                 ***REMOVED*** this paper
```
...
***REMOVED******REMOVED*** 8. Verdict

Universality class ***REMOVED***6 **cross-mechanism variant (***REMOVED***6b)** is **REJECTED** on the financial-versus-natural-disaster pair at daily frequency: empirical $\hat\lambda_U(0.95) = 0.052$ lies inside the independence null, Gumbel and survival-Clayton MLE collapse to the independence boundary, and the conditional baseline lift is $1.04$. The within-mechanism variant (***REMOVED***6a, e.g. volatility clustering) is confirmed by positive control ($\lambda_U = 0.21$ at $q=0.95$, Gumbel $\lambda_U = 0.138$). The recommended Layer 4 update is to split class ***REMOVED***6 into mechanism-coupled (supported) and cross-mechanism (rejected) sub-classes.
