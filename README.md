<p align="center">
  ...
  <p align="center">
    ...
    <a href="***REMOVED***run-the-included-regression-suite"><img alt="Tests" src="https://img.shields.io/badge/tests-48_backend_+_11_e2e-brightgreen.svg"></a>
...
***REMOVED******REMOVED******REMOVED*** 1. SOC validation pipeline (`v4/lib/soc_pipeline.py`)
...
***REMOVED******REMOVED******REMOVED*** 2. Phase Detector product (`web/phase-detector/` + `phase/`)
...
***REMOVED******REMOVED******REMOVED*** 3. Cross-judge methodology (`v4/critics/` + `web/`)
...
***REMOVED******REMOVED******REMOVED*** Run the SOC pipeline on a sample dataset

```python
from v4.lib.soc_pipeline import fit_clauset_powerlaw

***REMOVED*** fit a power-law tail with Clauset MLE + xmin selection
...
***REMOVED******REMOVED******REMOVED*** Verify thirteen systems at once

```bash
v4 status                      ***REMOVED*** show pass/fail across all 13 systems + 4 nulls
v4 validate soc-earthquakes    ***REMOVED*** re-run a single system
v4 list                        ***REMOVED*** enumerate all registered systems
```

***REMOVED******REMOVED******REMOVED*** Run the included regression suite

```bash
.venv/bin/python -m pytest v4/tests/sanity -m sanity -q
***REMOVED*** 38 sanity tests, ~3.6s — guards exponent bands, hash invariants,
***REMOVED*** null-control rejection, schema validation.
```

***REMOVED******REMOVED******REMOVED*** Reproduce a validation paper
...
├── v4/                     ***REMOVED*** primary research pipeline (Layers 1-5)
│   ├── lib/soc_pipeline.py ***REMOVED*** the shared 339-LOC Clauset pipeline
│   ├── critics/            ***REMOVED*** multi-model LLM ensemble (B1 / B3 / B4)
│   ├── taxonomy/           ***REMOVED*** 35-class per-class YAML taxonomy
│   ├── tests/              ***REMOVED*** 213 unit + integration + e2e tests
│   ├── results/            ***REMOVED*** frozen verdicts per system
│   └── cli.py              ***REMOVED*** `v4` console entry point
├── web/                    ***REMOVED*** production websites
│   ├── frontend/           ***REMOVED*** beta.structural.bytedance.city (search engine)
│   ├── backend/            ***REMOVED*** FastAPI + SSE Perplexity-style /api/ask/stream
│   └── phase-detector/     ***REMOVED*** phase.bytedance.city (Next.js 14)
├── paper/                  ***REMOVED*** arXiv-format preprints (v0 unified + 4 solo)
├── dataset/v1/             ***REMOVED*** frozen dataset bundle (mintable Zenodo DOI)
├── tutorials/              ***REMOVED*** Jupyter reproduction notebooks
├── plans/                  ***REMOVED*** research plan documents (Layer 5+, B4+, ...)
└── docs/                   ***REMOVED*** engineering docs (deployment, security, sessions)
```
