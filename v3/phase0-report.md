...
***REMOVED******REMOVED******REMOVED*** ✅ 可行性：StructTuple 抽取管道跑通

- Kimi 和 Opus 都能按 schema 输出 **零格式错误**
- 每样本耗时 Kimi ~7s / Opus ~10s
- 50 样本全量跑完 Kimi ~6 分钟 / Opus ~8 分钟

***REMOVED******REMOVED******REMOVED*** ⚠️ 重大发现：40% 的现象**不是动力学系统**
...
***REMOVED******REMOVED******REMOVED*** ✅ 好消息：60% 动力学子集的交叉匹配质量很高
...
***REMOVED******REMOVED******REMOVED*** ⚠️ 警示：Kimi vs Opus 一致率仅 64%
...
***REMOVED******REMOVED*** Schema 需要调整的地方
...
***REMOVED******REMOVED*** 决策点：Phase 0 → Phase 1 走哪条路

***REMOVED******REMOVED******REMOVED*** 方案 A：窄口径 V3（推荐）⭐
...
***REMOVED******REMOVED******REMOVED*** 方案 B：宽口径 V3（并行结构+动力学）
- 加 `structural_pattern` 平行轴，两个维度都做匹配
- 4443 现象全部覆盖
- 优点：完整
- 缺点：schema 复杂度翻倍，Phase 1 再调 3-5 天，总时间 +1 周

***REMOVED******REMOVED******REMOVED*** 方案 C：V3 暂停
- Phase 0 已证明 60% 动力学子集有希望
- 但没有证明"V3 一定比 V2 好"
- 可以把资源转去写论文，V3 后续再说

***REMOVED******REMOVED*** 推荐路径
