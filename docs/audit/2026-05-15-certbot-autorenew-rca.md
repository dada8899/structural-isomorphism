***REMOVED*** Certbot Auto-Renew Failure — Root Cause Analysis
...
**Owner**: wanqh (B 机 session ***REMOVED***9 follow-up)

---

***REMOVED******REMOVED*** 1. Root Cause（一句话）
...
***REMOVED******REMOVED*** 2. 证据链

***REMOVED******REMOVED******REMOVED*** 2.1 Timer 状态（修复前）
...
***REMOVED******REMOVED******REMOVED*** 2.2 Cron 也空
...
***REMOVED******REMOVED******REMOVED*** 2.3 Journal 零历史
...
***REMOVED******REMOVED******REMOVED*** 2.4 LE 日志佐证
...
***REMOVED******REMOVED*** 4. 当前修复状态

***REMOVED******REMOVED******REMOVED*** 已完成（2026-05-15 11:53 CST）

```bash
ssh vps "systemctl enable --now certbot-renew.timer"
***REMOVED*** Created symlink /etc/systemd/system/timers.target.wants/certbot-renew.timer
***REMOVED***                  → /usr/lib/systemd/system/certbot-renew.timer

ssh vps "systemctl status certbot-renew.timer"
***REMOVED*** ● certbot-renew.timer
***REMOVED***      Loaded: loaded (...; enabled; preset: disabled)    ← enabled ✓
***REMOVED***      Active: active (waiting) since 2026-05-15 11:53:35 CST  ← active ✓
***REMOVED***     Trigger: 2026-05-15 16:51:41 CST                    ← 4h 58min later
```
...
***REMOVED******REMOVED******REMOVED*** 待验证（dry-run 进行中）
...
***REMOVED******REMOVED*** 5. 防再发措施（系统性根因修复）

按 CLAUDE.md「四层分析」第 3 层（系统性根因）+ 第 4 层（全局影响评估）思路：

***REMOVED******REMOVED******REMOVED*** M1. 监控层补告警（最高优先级）
...
***REMOVED******REMOVED******REMOVED*** M2. cc-daemon 增加 weekly self-check（次优先级）
...
***REMOVED******REMOVED******REMOVED*** M3. 新 VPS 部署 checklist 加一条（防未来重装）
...
***REMOVED******REMOVED*** 6. 经验沉淀（写入 Memory）
...
***REMOVED******REMOVED*** Appendix B: 完整证书清单（34 个）
