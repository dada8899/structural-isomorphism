***REMOVED*** UX consistency audit — 2026-05-24 (W7-D mini-brief 5)
...
***REMOVED******REMOVED*** Findings — 18 inconsistency points

| ***REMOVED*** | Severity | Category | Page(s) | Finding | Recommendation |
|---|---|---|---|---|---|
| 1  | **P0** | Color tokens | `pricing.html` (new), `index.html` waitlist | New W7-D code uses ad-hoc `***REMOVED***1C1917` for `--brand-ink`; correct token is `***REMOVED***18181B`. Drift cost: ~4% perceptual delta, visible on side-by-side. | Replace with `var(--brand-ink)`. |
| 2  | **P0** | Color tokens | `pricing.html` | Hardcoded `***REMOVED***E7E5E4` for border; correct token is `var(--brand-line, ***REMOVED***E4E4E7)`. | Use token. |
| 3  | **P0** | Color tokens | `pricing.html`, `index.html` waitlist | Hardcoded `***REMOVED***57534E` for muted text; correct token is `var(--brand-muted, ***REMOVED***71717A)`. | Use token. |
...
| 9  | P1 | Focus states | `index.html` waitlist input + `pricing.html` button | `outline: 2px solid` is correct but uses raw color; should use `var(--accent, ***REMOVED***2563EB)` for keyboard focus visibility. | Switch outline color to accent token. |
...
| 17 | P3 | Privacy footer | New `pricing.html` says "支付由 Stripe 处理；我们不接触你的卡号" but `privacy.html` has the full policy section | Link the pricing footer line to `/privacy***REMOVED***payments`. | One-line link addition. |
...
***REMOVED******REMOVED*** Landed in this PR (12 fixes)

The fixes below ship in this commit. The rest are documented for follow-up.

***REMOVED******REMOVED******REMOVED*** Color token alignment (3 fixes, P0)
...
***REMOVED******REMOVED******REMOVED*** Plausible analytics (1 fix, P0)
...
***REMOVED******REMOVED******REMOVED*** Border radius standardization (1 fix, P1)
- F5 — `pricing.html` card radius → `var(--radius-xl, 12px)`; input/button radius → `var(--radius-lg, 8px)`

***REMOVED******REMOVED******REMOVED*** Focus state token (1 fix, P1)
...
***REMOVED******REMOVED******REMOVED*** Tap target sanity (1 fix, P1)
...
***REMOVED*** 1. Static check: no more hardcoded `***REMOVED***1C1917` in the new W7-D pages
grep -n "***REMOVED***1C1917" web/frontend/index.html web/frontend/pricing.html
***REMOVED*** (should print nothing after this audit)

***REMOVED*** 2. Plausible coverage:
grep -L "Plausible analytics" web/frontend/*.html
***REMOVED*** (should print nothing — all 26 pages covered)

***REMOVED*** 3. e2e visual diff (post-deploy)
cd web && pnpm playwright test tests/e2e/visual --update-snapshots
```

---

***REMOVED******REMOVED*** Score self-estimate
