...
number is retracted):
  ...
  Abstract — UPDATED to reflect 27 → 45+ SOC validation systems,
    KB 4888 → 5388 entries, the v0.4 verdict matrix, and the
...
| W2A.3 | `tail_copula_contagion` | REJECT (2 prior) | Δλ(stress − calm) > 0.15 | Δλ ∈ [−0.006, +0.001]; Gumbel BIC win 999–3,224 | REJECT-CONFIRMED (3rd verdict) | Static tail-dependence beats stress/calm split; copula property |
...
| W2B.2 | `percolation_connectivity` | KEEP | Fisher τ ∈ [1.95, 2.15] (textbook 187/91) | τ = 1.94 with FSS collapse | PASS + SPLIT vs SF | 2D-lattice exponent distinct from scale-free percolation |
...
| W2C.5 | `anderson_localization` | KEEP | ν ∈ [1.50, 1.65] (textbook 1.572) | ν = 1.620 across two band regimes | PASS-CONFIRMED | 3D Anderson model FSS collapse |
...
| `tail_copula_contagion` | Gumbel BIC win 999–3,224 vs SOC | "Calm" vs "stress" *not separable*; static copula adequate | A copula is a marginal-stripped tail property, not a mechanism class (C4 paper §4.2) |
...
The v0.4 §3.5 increment is consciously conservative in three respects. We state each explicitly so a reviewer can place the result correctly.
...
- Abstract rewritten to reflect 27 → 45+ SOC validation systems, KB 4888 → 5388 entries, the 18-class verdict matrix, the Layer-0 descriptor cluster, and the Spence-signal / OZ-Lorentzian surprises.
