***REMOVED*** 起手:structural-isomorphism / M1.2-M1.3 实测后续

> Session ***REMOVED***15 起手 prompt(浓缩版)。完整交接见 `SESSION-14-HANDOFF.md`,M1 实施依据见 `SESSION-13-M1-experience-fix-research.md`。

**项目**:`~/Projects/structural-isomorphism/`(repo: `dada8899/structural-isomorphism`, PUBLIC)
**上一 session**:***REMOVED***14。

***REMOVED******REMOVED*** 当前状态

- **PR ***REMOVED***224**(session ***REMOVED***14, `feat/m1-ask-experience-fix`, 5 commits, 47 测试全绿):
...
- **PR ***REMOVED***222**(CI 修复, 全绿)、**PR ***REMOVED***223**(首页搜索框)同样 OPEN
- main HEAD 仍是 session ***REMOVED***12 的 commit

***REMOVED******REMOVED*** 用户应已完成(起手先确认)

```bash
cd ~/Projects/structural-isomorphism && git fetch && git log --oneline origin/main -5
```

若 ***REMOVED***222/***REMOVED***223/***REMOVED***224 已 merge:✅ 进入"下一步";若未 merge:停下提醒用户先 squash merge,不要在过期 base 上开新工作。

***REMOVED******REMOVED*** 下一步分叉(看 :nitro 实测 TTFT 数据)
...
***REMOVED******REMOVED*** 实测脚本骨架(可选,VPS 上跑)

```bash
***REMOVED*** 7 条 dogfood query 跑 :nitro 后的 TTFT
