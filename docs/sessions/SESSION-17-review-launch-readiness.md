...
### P1-4 `/docs` 与 `/openapi.json` 在 prod 公开可访问
...
### P1-5 API 错误响应格式不统一（部分非 RFC 7807）
...
### P1-7 robots.txt 允许搜索引擎收录 beta 站
...
### P2-3 `/api/version` 在 `STRUCTURAL_GIT_SHA` 未设时 fork `git` 子进程
...
### P2-4 无 favicon / apple-touch-icon 检查项
...
### P2-6 `phase/*` 端点用 `urllib` 同步阻塞调用 + `include_in_schema=False`
