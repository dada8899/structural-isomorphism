# Session #18 交接 — 价值挖掘 A–G

> 上一 session：#17。本文是给下一个 CC session 的技术交接。
...
## 0. 当前 prod 状态（verified 2026-05-22）

- 站点：`https://beta.structural.bytedance.city`
- prod git_sha：`3abaa78`（session #17 全部成果已上线）
...
```
看到 ✅ 才往下走。看不到立刻怀疑 deploy 没真上线（session #15 血泪）。
注意：`_local_git_sha` 已修成「取最近非 docs commit」，docs-only 提交不会误报 mismatch。

**session #17 已完成（别重做）**：M1.4 e2e、My Reports 页、发布前 5 路审查 + P0/P1/P2 全修、价值优化 V1–V6、nginx SSE 修复、搜索框去蓝光环。详见 `progress.md` 和 `SESSION-17-*.md` 系列。

---

## 1. 本 session 任务：价值挖掘 A–G

A–G 是 7 个**产品方向**（不是 7 个小功能）。一个 session 做不完全部 —— **按下方「执行排序」做**，便宜高杠杆的先做扎实，长线的给起步切片。不要每个都做一半。

### 资产清单（A–G 都建立在这些之上，先摸清）
...
- 回访数据：`report_followup` 表（session #17 V6 新建）+ `report_feedback` 表
...
## B. 数据飞轮 —— 把「收集真实结果」变成核心设计 ★最重要、最该认真对待
...
- 注意：session #17 已把这些页从主导航移除（只留核心 4 页）；做 D 时要决定是否重新给它们导航入口。

---

## E. verification-as-a-service —— 把「证伪能力」单独做成产品
...
做完 1–4 就是非常扎实的一个 session。5–7 看时间，做不完顺延 #19。**宁可 1–4 做透，不要 1–7 都做一半。**

---

## 3. 工程纪律（session #17 验证有效，照做）
...
- **nginx SSE 已修**（session #17）：`/api/(analyze|ask)/stream` 已有专门 location（buffering off + 600s timeout），报告能跑完。VPS 配置备份在 `/etc/nginx/conf.d/beta-structural.conf.bak-s17`。若再动 nginx 必须 `nginx -t` 通过才 reload。
...
| 优先级 | 动作 | 说明 |
|---|---|---|
| 🔴 P0 | 轮换 OpenRouter API key | `web/backend/.env.bak-v1` 的 key 曾在 PUBLIC 仓库泄露（session #17 已 untrack 但 key 已暴露）。去 OpenRouter 控制台轮换 |
