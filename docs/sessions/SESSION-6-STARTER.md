***REMOVED*** Session ***REMOVED***6 Starter — 交接文档

> Session ***REMOVED***5 完成时间：2026-05-14 12:30 CST（约）
> Main HEAD：`4b9f888`
> 累计 PRs (session ***REMOVED***5)：9 个 merged (***REMOVED***54-***REMOVED***62)
> 用户身份：达达 (dada8899)，repo PUBLIC
>
> Session ***REMOVED***6 起手第一件事：读本文件 §0，运行起手扫描，然后按 §3 的优先级队列推进。

---

***REMOVED******REMOVED*** §0 起手 60 秒 — 环境 + 同步扫描

```bash
cd ~/Projects/structural-isomorphism
git pull origin main
git log --oneline -5         ***REMOVED*** HEAD 应该是 4b9f888 或更新
gh auth status               ***REMOVED*** 必须 ✓ logged in dada8899
ls .env                      ***REMOVED*** 本地 DeepSeek key（gitignored）
```

***REMOVED******REMOVED******REMOVED*** 凭证状态

| 凭证 | 位置 | 状态 |
|---|---|---|
| DeepSeek API key (rotated) | `~/Projects/structural-isomorphism/.env` | ✅ 本地 |
| DeepSeek key on VPS | `systemctl show phase-detector-api -p Environment` | ⚠️ 未确认（用户 rotate 后 VPS 端可能仍是旧 key，session ***REMOVED***5 没碰）|
...
***REMOVED******REMOVED******REMOVED*** 关键路径（活的产线）
...
> **⚠️ 注意路径区分**：rsync 部署去的是 `web/frontend/`，活的产线是 `beta.` 子域。`structural.` 主域是 4 月遗留的学术静态页，不要 rsync 到 `/site/`。Session ***REMOVED***5 PR-1 部署时撞过这个坑（详见 §6 quirks）。

***REMOVED******REMOVED******REMOVED*** 部署命令（session ***REMOVED***5 验证过）

```bash
***REMOVED*** Structural 前端 + 后端 → beta.structural.bytedance.city
cd ~/Projects/structural-isomorphism
rsync -av --delete --exclude='.DS_Store' web/frontend/ \
  root@43.156.233.71:/root/Projects/structural-isomorphism/web/frontend/

***REMOVED*** 后端改了的话也 rsync
...
***REMOVED*** Phase Detector → phase.bytedance.city
...
***REMOVED******REMOVED*** §1 Session ***REMOVED***5 落地清单（不要重复做）

9 个 PR 全部 merged 到 main + 部署上线 + smoke 通过：

| PR | 主题 | Files / LOC |
|---|---|---|
| ***REMOVED***54 | TL;DR 卡片 + /synthesize SSE + footer "HuggingFace 数据集" | 20 files, +669/-61 |
| ***REMOVED***55 | 共享品牌 token (5-var canonical) + SSE markdown fence-strip | 22 files, +547/-41 |
| ***REMOVED***56 | StatsBar skeleton — 干掉 "正在加载统计…" flash | 1 file, +38/-2 |
| ***REMOVED***57 | GlossaryTooltip — 7 jargon hover 解释 (auto-wrap) | 13 files, +413 |
| ***REMOVED***58 | /search 立即出卡，不等 synth | 2 files, +43/-19 |
| ***REMOVED***59 | daily-grid skeleton + sector dropdown disabled-while-loading | 4 files, +96/-26 |
| ***REMOVED***60 | hero-evidence__desc line-clamp:3 + min-height (无高度抖动) | 1 file, +11 |
| ***REMOVED***61 | /analyze 在 meta arrival 隐藏 loading block | 2 files, +26/-14 |
| ***REMOVED***62 | 修 PR-8 在 error/retry 路径的 regression | 3 files, +41/-18 |

**总计**：68 文件改动，+1884 / -181 LOC。

***REMOVED******REMOVED******REMOVED*** 已生效的能力
...
***REMOVED******REMOVED******REMOVED*** Multi-dimensional 打分（粗估）

session ***REMOVED***4 末 84.4% → session ***REMOVED***5 末预计 **88-90%**（+4-6 pts），主要在：
- 流式生成体验（synth 端从 "等完才出" → "字符流"）
- 首屏可见性（TL;DR 卡 + instant cards）
- Layout 稳定性（4 处 CLS 修）
- Jargon 可学习性（GlossaryTooltip）

session ***REMOVED***6 起手如果走多 agent verification，可补充打分。

---

***REMOVED******REMOVED*** §2 Defer 到 session ***REMOVED***6 的事

按重要性排序：

***REMOVED******REMOVED******REMOVED*** P0 — 发布路径（等用户给凭证）
...
1. **PyPI publish — `soc-pipeline` + `guarded-llm`**
   ...
   - 用户给 `PYPI_TOKEN` 后，session ***REMOVED***4 SESSION-4-STARTER.md §2 Step 5 直接跑
...
***REMOVED******REMOVED******REMOVED*** P1 — UX 第三轮（自助可推进，无凭证依赖）

6. **/analyze 端到端真实环境测试**
   - PR ***REMOVED***54-***REMOVED***62 全部部署但 session ***REMOVED***5 没用浏览器跑过完整流（只 curl SSE 头几个 chunk）
...
10. **/discoveries 页 layout shift**
    - PR-6 修了 homepage 的 daily-grid，但 `/discoveries` 完整列表页 (***REMOVED***disc-hero-stats / ***REMOVED***disc-filter / ***REMOVED***disc-list) 没修
    - 同模式 skeleton 抽出去服用即可（PR-6 新加的 `.skeleton-bar` 可复用）
    - 估计 30 min

***REMOVED******REMOVED******REMOVED*** P2 — 长期 staged

11. **GlossaryTooltip 第二批词汇**（content review 后）
    - 当前只 7 词。session ***REMOVED***5 选词基于 grep "在用户页出现"
...
***REMOVED******REMOVED*** §3 Session ***REMOVED***6 起手推荐顺序
...
3. 用户决策：本 session 走哪条路？
   A. 发布优先（P0 ***REMOVED***1-***REMOVED***5 + ***REMOVED***4）— 需要用户先给 token
   B. UX 第三轮（P1 ***REMOVED***6-***REMOVED***10）— 自助可推
   C. 双轨：先做 P1 ***REMOVED***6+***REMOVED***7 验证 session ***REMOVED***5 的 9 PRs 没 regression，
            等用户给 token 中途再切发布
```

**默认推荐 C**：先 30min Playwright e2e 跑 /analyze + /search，确认 PR ***REMOVED***54-***REMOVED***62 真实可用 + 截图存档，再视用户授权切别的轨。

---

***REMOVED******REMOVED*** §4 项目结构速查（session ***REMOVED***5 后）

```
~/Projects/structural-isomorphism/
├── web/
│   ├── frontend/              ***REMOVED*** Structural 静态 HTML+JS (14 user pages + 3 researcher)
│   │   ├── index.html         ***REMOVED*** 首页（hero-evidence 自动轮播 + daily-grid skeleton + cross-product）
│   │   ├── classes.html       ***REMOVED*** 共享模式
│   │   ├── search.html        ***REMOVED*** 搜索结果 (instant cards + SSE typewriter)
│   │   ├── analyze.html       ***REMOVED*** 深度分析 (TL;DR card + 9 sections, action_plan first)
│   │   ├── discoveries.html, methods.html, about.html, papers.html, ...
│   │   ├── start-here.html    ***REMOVED*** /start-here 长文
│   │   └── assets/
│   │       ├── js/            ***REMOVED*** i18n / utils / api (SSE) / search / analyze / glossary / 
│   │       │                    history-sidebar / home / classes / discoveries / phenomenon
│   │       └── css/           ***REMOVED*** design-system / common / search / analyze / glossary / 
│   │                            shared-tokens (mirror) / responsive / home
│   ├── phase-detector/        ***REMOVED*** Phase Detector Next.js (phase.bytedance.city)
│   │   ├── app/page.tsx       ***REMOVED*** 首页（含 SearchHero）
│   │   ├── app/layout.tsx     ***REMOVED*** 顶栏 + footer + HistorySidebar
│   │   ├── app/company/[ticker]/page.tsx
│   │   ├── app/{about,methodology,thank-you}
│   │   ├── app/globals.css    ***REMOVED*** 引 ./shared-tokens.css (mirror) + Tailwind
│   │   ├── app/shared-tokens.css  ***REMOVED*** ← byte-identical mirror of web/shared/tokens.css
│   │   ├── components/{SearchHero, HistorySidebar, ScreenerFilter, StatsBar, ...}
│   │   └── lib/{types, labels, api, parse-query, history, mock-data}.ts
│   ├── shared/                ***REMOVED*** ← session ***REMOVED***5 PR-2 新增
│   │   ├── tokens.css         ***REMOVED*** canonical 5-var brand palette (--brand-ink/paper/accent/line/muted)
│   │   ├── sync-tokens.sh     ***REMOVED*** cp tokens.css 到 frontend + phase-detector mirrors，verify byte-identical
│   │   └── README.md
│   └── backend/               ***REMOVED*** FastAPI (port 5004)
│       ├── main.py
...
├── packages/
│   ├── soc-pipeline/dist/     ***REMOVED*** PyPI artifacts (twine check PASSED, 待 publish)
│   └── guarded-llm/dist/
├── dataset/v1/                ***REMOVED*** 99MB, 244 文件, 待 Zenodo
├── paper/
│   ├── v0-unified-pipeline-2026-05-13.md  ***REMOVED*** C1 unified v0.3.1
│   ├── arxiv-drafts/2026-05-13/01..04_*.md
│   └── c4-reject-aware-pipeline-2026-05-13.md
└── docs/sessions/
    ├── HANDOFF.md
    ├── SESSION-4-STARTER.md   ***REMOVED*** 仍 valid 的 publishing runbook (PyPI/Zenodo/arXiv 步骤)
    ├── SESSION-5-STARTER.md   ***REMOVED*** session ***REMOVED***4 末 → ***REMOVED***5 的入口
    └── SESSION-6-STARTER.md   ***REMOVED*** 本文件
```

---

***REMOVED******REMOVED*** §5 不可逆操作清单（session ***REMOVED***6 任何动作前确认）
...
***REMOVED******REMOVED*** §6 已知 quirks（session ***REMOVED***5 新增 + 老的）

***REMOVED******REMOVED******REMOVED*** 老的（session ***REMOVED***4 留下）
1. **/index.html 404**（clean URL `/` 200）— canonical URL 唯一，非 bug
2. **/start-here.html 404**（clean URL `/start-here` 200）— 同上
3. **paper.html / papers.html / taxonomy-v2.html 保留物理 jargon + LaTeX** — researcher-facing 例外，PR-1 §F style guide。**Session ***REMOVED***5 PR-4 的 GlossaryTooltip 也跳过这 3 页**
...
***REMOVED******REMOVED******REMOVED*** Session ***REMOVED***5 新增

7. **`structural.bytedance.city` 主域 ≠ 活的产线**：主域 nginx serves `/root/Projects/structural-isomorphism/site/` (4 月学术静态页，仅 / + paper-zh.html 等)。活的产线是 `beta.structural.bytedance.city`，FastAPI 服务 5004 端口。Session ***REMOVED***5 PR-1 部署时撞过这个坑——`curl https://structural.bytedance.city/api/synthesize/stream` 返回 405 不是 bug，因为主域 nginx 不 proxy /api。

8. **rsync 多源单 dest 会失败**：`rsync a.py b.py target/path.py` —— 只 rsync 一个文件或整目录。Session ***REMOVED***5 PR-1 部署时撞到，Backend 改 2 个文件需 2 次 rsync。
...
11. **`web/shared/tokens.css` 改了必须跑 sync-tokens.sh**：3 处文件（canonical + 2 mirror）必须 byte-identical。CI 没强制（TODO），靠人肉纪律。修改 brand color 时务必：
    ...
    ./web/shared/sync-tokens.sh   ***REMOVED*** 自动 cp + verify
    git add web/shared/tokens.css web/frontend/assets/css/shared-tokens.css web/phase-detector/app/shared-tokens.css
    git commit
    ```

12. **`***REMOVED***analyze-loading` 在 meta arrival 后 hidden 不 removed**（PR-9 修 PR-8 regression）：error / retry 事件需要这个节点存在以重新画错误。如果未来重构 analyze.js，**不要轻易 .remove() 这个节点**，hidden 即可。
...
***REMOVED******REMOVED*** §7 Session ***REMOVED***6 起手 prompt 模板

如果用户想换 prompt 直接告诉 session ***REMOVED***6 Claude：

```
[option C — 推荐：双轨]
读 docs/sessions/SESSION-6-STARTER.md。
按 §3 起手扫描 + §2 P1 ***REMOVED***6 ***REMOVED***7 用 Playwright MCP 跑一遍 /analyze + /search e2e
（验证 session ***REMOVED***5 9 个 PR 真实可用），存截图。
等我准备好 token 再切发布路径。

[option B — 自助 UX 第三轮]
读 docs/sessions/SESSION-6-STARTER.md。
按 §3 直接做 P1 ***REMOVED***6 ***REMOVED***7 ***REMOVED***8（e2e + mobile audit），
有时间继续 ***REMOVED***9 ***REMOVED***10。全权 auto mode。

[option A — 发布路径]
读 docs/sessions/SESSION-6-STARTER.md。
我准备好了 PYPI_TOKEN=xxx 和 ZENODO_ACCESS_TOKEN=xxx。
按 §2 P0 ***REMOVED***1-***REMOVED***5 顺序跑（先验证 /tmp artifact 还在，
不在的话先重 build + 重打包 dataset）。
```

---

**Session ***REMOVED***5 结束**。HEAD `4b9f888`，9 PRs，UX 总分预计 84.4% → 88-90%（+4-6 pts）。
