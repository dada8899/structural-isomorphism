***REMOVED*** Session ***REMOVED***12 — Pre-launch multi-agent audit (W16)
...
***REMOVED******REMOVED*** 1. Five-agent review summary

Five read-only review agents ran in parallel against the pre-launch repo
state at commit `e3ceab1` (Session ***REMOVED***11 closeout). Findings rolled up to
...
***REMOVED******REMOVED*** 2. P0 fixes that landed

***REMOVED******REMOVED******REMOVED*** Security (architect)
...
***REMOVED******REMOVED******REMOVED*** Copy / positioning (UX + PM + real-user)
...
***REMOVED******REMOVED******REMOVED*** Number consistency (scholar)
...
***REMOVED******REMOVED******REMOVED*** Scholarship (scholar)
...
***REMOVED******REMOVED******REMOVED*** Launch posts
...
***REMOVED******REMOVED******REMOVED*** Architect P1 housekeeping
...
***REMOVED******REMOVED*** 3. P1 items still open

***REMOVED******REMOVED******REMOVED*** 3.1 Scholar — cross-domain vs within-class framing in the papers
...
***REMOVED******REMOVED******REMOVED*** 3.2 Scholar — Anderson / Preisach falsifiability
...
***REMOVED******REMOVED******REMOVED*** 3.3 Scholar — FWER correction surfaced in papers
`v4/results/F3_fwer_corrected.jsonl` exists but the antipage paper §3.5
doesn't quote Bonferroni-corrected p-values. Reviewer ***REMOVED***2 will ask.

***REMOVED******REMOVED******REMOVED*** 3.4 PM — academic citation surface on the web
...
***REMOVED******REMOVED******REMOVED*** 3.5 Architect — Dep upper bounds + repo housekeeping
...
***REMOVED******REMOVED******REMOVED*** 3.6 UX — small polish
...
- Dark-mode hero CTA uses hardcoded `bg-indigo-700` — won't flip to
  the `***REMOVED***A78BFA` dark accent. Use semantic `bg-accent text-accent-fg`.

---

***REMOVED******REMOVED*** 4. Tests touched in this pass
...
***REMOVED******REMOVED*** 5. Residual irreversible-action checklist (for the human operator)

***REMOVED******REMOVED******REMOVED*** 5.1 Rotate the exposed OpenRouter key + scrub history
The key `sk-or-v1-af9ae735…aea878` was committed in
`web/backend/.env.bak-v1` and remains in `git log`.

```bash
***REMOVED*** 1. Rotate at https://openrouter.ai/keys (revoke the old key; mint a new one).
***REMOVED*** 2. Update STRUCTURAL_DEPLOY env files on the VPS with the new key.
***REMOVED*** 3. Scrub the file from history before flipping PUBLIC:
git filter-repo --path web/backend/.env.bak-v1 --invert-paths
***REMOVED***    (or BFG: bfg --delete-files .env.bak-v1)
***REMOVED*** 4. Force-push to your remote AFTER you have confirmed all team members
***REMOVED***    have stopped pushing to the affected branches.
```

***REMOVED******REMOVED******REMOVED*** 5.2 PyPI publish (3 packages)
With `PYPI_TOKEN` set:
...
***REMOVED******REMOVED******REMOVED*** 5.3 Zenodo DOI mint
With `ZENODO_ACCESS_TOKEN` set, follow `docs/sessions/SESSION-4-STARTER.md`
§2.5.

***REMOVED******REMOVED******REMOVED*** 5.4 arXiv submission (5 papers)
...
***REMOVED******REMOVED******REMOVED*** 5.5 GitHub repo PUBLIC flip
...
***REMOVED******REMOVED******REMOVED*** 5.6 GH Pages
Enable for mkdocs (docs/) and Storybook (web/phase-detector/) from
Settings → Pages.

---

***REMOVED******REMOVED*** 6. What this audit did NOT touch (intentionally)
