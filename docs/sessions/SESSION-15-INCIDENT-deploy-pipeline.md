# SESSION-15 事故复盘:Deploy 哑炮 5 天 + 连锁 5 个问题

> 日期:2026-05-20。触发场景:PR #224 (M1.2/M1.3) merge 后做 dogfood TTFT 验证,数据反常 → 顺藤摸下去发现 deploy pipeline 5 天哑炮 + 连锁 4 个长期债。

## TL;DR

PR #224 合并后,从主战场( `dogfood_ttft.py` 测 7 条 query)反推出 5 个相互嵌套的问题。最深层的 **deploy 哑炮** 让上层每个症状(数据反常、模型不对、event 没 emit)都被错误归因。下次起手,**dogfood 测出反常 → 不要先怀疑代码,先怀疑 deploy 是不是真上线**。

---

## 5 个问题(从根因到表象)

### #1 — P0 Deploy infra 哑炮 5 天 ❌→✅
...
**修复**:见 task #8。需要再加一条 deploy key 走另一个 `command=` 限制(指向 `deploy-vps.sh`),让 backend workflow 用新 key。当前权宜:手动 `ssh + bash scripts/deploy-vps.sh` 触发真部署。
...
### #2 — P0 `requirements.txt` 漂移到 prod runtime ❌→✅
...
### #3 — P1 `ASK_LLM_MODEL` env 覆盖让 :nitro 从未生效 ❌→pending

**症状**:dogfood 第一轮的 SSE `meta` event 显示 `"model": "anthropic/claude-sonnet-4.6"`,不是 PR #224 commit `2936f4d` 改的 `deepseek/deepseek-chat:nitro`。
...
VPS 上 `web/backend/.env` 里有 `ASK_LLM_MODEL=anthropic/claude-sonnet-4.6`,**完全覆盖** PR 默认值。即使 deploy 没哑炮(#1),`:nitro` 的 TTFT 优化也不会生效。

**修复**:Task #9。`unset ASK_LLM_MODEL` 或者从 `.env` 删掉那行,让 PR 默认值生效。
...
### #4 — P1 第一轮 dogfood 数据完全无效 ❌→需重跑

**结论**:第一轮 `docs/sessions/session-15-ttft.json` 的数据**整份扔掉**。打的是 5 天前的旧代码 + Claude Sonnet,不是 PR #224 的 :nitro + DeepSeek + M1.3 短路。Verdict "max=11.36s → 启 Fix2/3" **不能信**。
...
### #5 — P2 测试漏断言 SSE event(漏过 #224 review)❌→✅

**症状**:`test_ask_streaming.py::test_full_event_sequence_streaming_path` 验了 retrieval_done / kb_cards / answer_chunk / answer_done / similar_phenomena / followups / done,**唯独没断言 `llm_start`**。所以即使 PR #224 commit `5090e4c` 真没写 `yield _sse("llm_start", ...)` (实际写了,但流程上是漏的),49 个测试还能全绿。`test_structlog_format.py` 里有一个 `ask.llm.start` 引用,但那只验证 log 行格式,跟 SSE event 完全不同。
...
## 长效防御:加一条 deployment fingerprint check
...
dogfood 脚本起手第一件事是抓这个 endpoint + 跟 GH Actions workflow ID 对账,**不对上不跑 dogfood**。这把 #1 + #3 + #4 三条链路全锁住。
