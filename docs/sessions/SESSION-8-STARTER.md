***REMOVED*** Session ***REMOVED***8 Starter

> Session ***REMOVED***7 完成时间：2026-05-14 ~15:30 CST
> Main HEAD: `23484f7`
> 累计 PRs in session ***REMOVED***7: **15** (***REMOVED***64-***REMOVED***79)
> 用户身份：达达 (dada8899)，repo PUBLIC
>
> Session ***REMOVED***8 第一件事：读本文件 §0，跑起手扫描，然后按 §3 优先级队列推进。
...
git log --oneline -8         ***REMOVED*** HEAD 应该是 23484f7 或更新
gh auth status               ***REMOVED*** 必须 ✓ logged in dada8899
ls .env                      ***REMOVED*** 本地 DeepSeek key
.venv/bin/python -m pytest web/backend/tests/ -q   ***REMOVED*** 30 backend tests 应全过
.venv/bin/python -m pytest web/tests/e2e/ -q       ***REMOVED*** 11 e2e 应全过
curl -s -o /dev/null -w "%{http_code}\n" https://beta.structural.bytedance.city/   ***REMOVED*** 应 200
```
...
| GitHub auth | keyring | ✅ |
| PYPI_TOKEN | — | ❌ session ***REMOVED***6 P0 ***REMOVED***1 仍待用户 |
| ZENODO_ACCESS_TOKEN | — | ❌ session ***REMOVED***6 P0 ***REMOVED***2 仍待用户 |
| arXiv account | — | ❌ session ***REMOVED***6 P0 ***REMOVED***3 仍待用户 web |
| Sentry DSN | — | ❌ Sentry SDK 集成 ready，DSN 不存在则 no-op |
| NVD API key | — | ❌ session ***REMOVED***7 W2-C 跑 CVE 2023 用 unauth 24k records；2010-2025 全量需要 API key |
...
***REMOVED******REMOVED*** §1 Session ***REMOVED***7 落地清单（不要重复做）

✅ Session ***REMOVED***7 一次性 ship 5 个 direction + Perplexity-like 主页面 + 部署 + e2e 11/11 pass。完整 PR 表在 `HANDOFF.md` §1。

***REMOVED******REMOVED******REMOVED*** 已生效的新能力
...
***REMOVED******REMOVED*** §2 Defer 到 session ***REMOVED***8 的事

按优先级：

***REMOVED******REMOVED******REMOVED*** P0 — Token-dependent 发布动作（仍待用户）

1. **PyPI publish** — soc-pipeline + guarded-llm wheels 已 ready（session ***REMOVED***3）
2. **Zenodo DOI mint** — dataset/v1 已打包（session ***REMOVED***3）
3. **arXiv submission** — C1 v0.3.1 + 4 solo + C4 (6 PDFs)
4. **GitHub release v0.4.0** — session ***REMOVED***7 milestone tag（可用 `git tag v0.4.0 23484f7 && git push origin v0.4.0`，不依赖 token）

***REMOVED******REMOVED******REMOVED*** P1 — Phase Detector 商业化（无 token 依赖，session ***REMOVED***8 主菜）
...
***REMOVED******REMOVED******REMOVED*** P1 — Perplexity-like 增强（用户反馈驱动）
...
19. **Plausible 数据回看** — session ***REMOVED***7 ship 14 events，跑 1-2 周后看真实使用率

---

***REMOVED******REMOVED*** §3 Session ***REMOVED***8 起手推荐顺序

```
1. §0 起手扫描（git pull / pytest backend e2e / curl prod health）
2. 用户决策：本 session 走哪条路？

   Option A — Phase Detector 商业化（P1 ***REMOVED***5 ***REMOVED***6 ***REMOVED***7 ***REMOVED***8）
     需要外部数据源 (Stooq/yfinance 价格 API)。先跑完 500 ticker StructTuple
     做 walk-forward backtest，看真数据下 t-test 是否显著。

   Option B — 发布优先（P0 ***REMOVED***1 ***REMOVED***2 ***REMOVED***3）
     用户给 PYPI_TOKEN + ZENODO_ACCESS_TOKEN + arXiv 账号

   Option C — Perplexity 增强（P1 ***REMOVED***9-***REMOVED***13）
     用户反馈驱动，需要先看 prod 真实使用数据

   Option D — 科学纵深（P1 ***REMOVED***14 ***REMOVED***15 ***REMOVED***16）
     OpenRouter Kimi key 拿到 → 跑完 B4 23 类 → 论文级 verdict
```

---

***REMOVED******REMOVED*** §4 已知 quirks

继承 session ***REMOVED***6-STARTER §6 12 条 + 新加 6 条：

13. **slowapi 动态限速 callable 不收 request** — `__limit_provider()` 无参（或 key 单参）。tier-aware rate limit 要靠 contextvar 不是 closure。
14. **session ***REMOVED***7 form 4 silent fallback 仍然真实** — `isolation: "worktree"` flag 不可靠。subagent 主动 `git worktree add -b <branch> /tmp/<name>` 自救最稳。
...
17. **Mobile nav overflow 是 pre-existing 全站问题** — `.site-header__nav` 300px 在 375px viewport 越界。session ***REMOVED***7 加了 `@media (max-width: 480px) { .site-header__nav .site-header__nav-link { display: none } }` 兜底。
18. **VPS 部署管道（实证 SOP）** — 看 `docs/sessions/session-7-deploy-verified.md` §部署管道，rsync + ssh restart + curl smoke 三件套。

---

***REMOVED******REMOVED*** §5 起手 prompt 模板

***REMOVED******REMOVED******REMOVED*** Option A (Phase Detector 商业化)

```
继续 session ***REMOVED***7 留的 P1 ***REMOVED***5-***REMOVED***8。
...
***REMOVED******REMOVED******REMOVED*** Option B (发布)

```
继续 session ***REMOVED***7 P0 ***REMOVED***1-***REMOVED***4。用户给 PYPI_TOKEN + ZENODO_ACCESS_TOKEN（在对话首段贴）：
1. PyPI publish soc-pipeline + guarded-llm
2. Zenodo mint DOI for dataset/v1
3. arXiv 提交 (web 操作，user-driven)
4. GitHub release v0.4.0 (session ***REMOVED***7 milestone) — 即使没 token 也能做
```

***REMOVED******REMOVED******REMOVED*** Option C (Perplexity 增强)
...
***REMOVED******REMOVED*** §6 Session ***REMOVED***7 Stats

- **15 PRs merged in main** (***REMOVED***64-***REMOVED***79)
- **11 subagents** in 3 waves with worktree isolation
- **6 hotfix iterations** in main session (***REMOVED***75 ***REMOVED***76 ***REMOVED***78 + 3 manual cherry-pick rebase)
