***REMOVED*** Session ***REMOVED***9 起手交接文档

> 上 session (***REMOVED***8) 完成于 2026-05-14 ~22:30 CST
> Main HEAD: `c26df03` · 10 PRs merged (***REMOVED***87-***REMOVED***96)
...
git log --oneline -12                 ***REMOVED*** HEAD = c26df03 or newer
gh auth status                        ***REMOVED*** ✓ dada8899

***REMOVED*** Tests
PYTHONPATH=. .venv/bin/python -m pytest web/backend/tests/ -q   ***REMOVED*** 30 pass
PYTHONPATH=. .venv/bin/python -m pytest web/tests/e2e/ -q       ***REMOVED*** 11 pass
...
***REMOVED******REMOVED*** 1. Session ***REMOVED***8 已交付（10 PRs，2 waves，全部 squash-merged）

***REMOVED******REMOVED******REMOVED*** Wave 1 — 5 agents 并行（5 PR）

| PR | Agent | 交付 |
|---|---|---|
| *****REMOVED***87** | W1-A | **Phase Detector real-data walk-forward backtest**. yfinance 拉 497/500 SP500 ticker × 5y 月度价 + Stooq fallback. 54 snapshot × 6mo forward return. **t=-0.412, p=0.681 → alpha NOT significant**. 商业化 fork point — pivot 到 narrative / transparency. |
| *****REMOVED***88** | W1-C | README 322→197 行 external 重写（dev README 留 `docs/legacy-readme.md`）+ universality dupe-fix doc + Phase Detector React error 早已修(verify only) |
| *****REMOVED***89** | W1-D | CVE FAIL paper §1 + §5 prose 补完到 4610 字 + mkdocs scaffold（9 docs 文件 + nav）+ `.github/workflows/docs.yml` GitHub Pages auto-deploy |
| *****REMOVED***90** | W1-B | citation click → `/phenomenon/{id}` polish + aria-label + tooltip · history sidebar remote-sync 默认 ON（opt-out via `structural_disable_remote_history=1`）· `/discoveries` CLS 0.2186 → 0.1927（剩余 94% 是 Noto Serif SC font swap，下个 session 自托管解决） |
| *****REMOVED***91** | W1-E | B4 ensemble 用 DeepSeek 直连 3-config 重跑（代 OpenRouter Kimi）·cost **$0.058** · 但 52% unanimous → **weak heterogeneity proxy**, 真异构仍需跨 vendor（Kimi/Sonnet/Gemini） |

***REMOVED******REMOVED******REMOVED*** Wave 2 — 5 agents 并行（5 PR）

| PR | Agent | 交付 |
|---|---|---|
| *****REMOVED***92** | W2-D | **Phase Detector auto-deploy CI** (`.github/workflows/deploy-phase-detector.yml`) · 触发 push to main + paths `web/phase-detector/**` · VPS SSH key 用 command= 限制只能跑 deploy 脚本 · `.env.production` 缺失会 abort（防 2026-05-14 bug 复发）· GH secrets: VPS_DEPLOY_KEY/VPS_HOST/VPS_USER |
| *****REMOVED***93** | W2-A | **https://phase.bytedance.city/backtest 上线**（transparency report tone, NOT alpha screener）· server component + recharts 客户端 chart · nginx 加 exact-match location for `/api/backtest-result` + `/api/backtest-cumulative` 跳 :3210 Next.js（backup nginx conf saved） |
| *****REMOVED***94** | W2-C | **WSB posts pre-reg real fit**（arctic_shift 替代 dead Pushshift）· 6000 posts · **P2 cascade α=2.017 IN band [1.7, 2.3]** · P1 Omori p=0.18 OUT（**under-tested 非 falsified**，数据结构不匹配 pre-reg）· **PARTIAL verdict** · 彩蛋: 2021 GME 后 stationarity break 实证（α 2.02 → 1.61）|
| *****REMOVED***95** | W2-B | **cross-judge package 抽出**（`packages/cross-judge/`，**not published to PyPI**）· 1664 LOC · 37/37 tests · 2 examples · DeepSeek 直连 as default vendor |
| *****REMOVED***96** | W2-E | **VPS structural-web LLM 升级到 anthropic/claude-sonnet-4.6** for `/api/ask/stream`（per `ASK_LLM_MODEL` env）· 3-query AB 测 Sonnet 全面更好（cross-domain isomorphism explanations + grounded citations）· cost ~28x ≈ $4/day @100 q/day · 500 q/day 时上 tier-aware routing · `.env.bak-1778767603` 备份在 VPS |

---

***REMOVED******REMOVED*** 2. Session ***REMOVED***8 metrics

- **10 PRs merged** (***REMOVED***87-***REMOVED***96), no conflicts (all branched from same origin/main per wave)
...
***REMOVED******REMOVED*** 3. ⚠️ Wave 3 destructive ops — 部分完成,part remaining

***REMOVED******REMOVED******REMOVED*** ✅ 已完成
...
***REMOVED******REMOVED******REMOVED*** 🛑 Pending — 需要用户介入

1. **Git history credential scrub**
   - 发现 2 个真实 key 泄露在 history：
     - `sk-REDACTED-BY-SCRUB-20260524` — DeepSeek 直连（**当前活跃**）
     - `sk-or-v1-REDACTED-BY-SCRUB-20260524` — OpenRouter
   ...
   - 用户需手动 rotate 后再 scrub + force-push：
     ```bash
     ***REMOVED*** 1. 用户在 DeepSeek dashboard rotate key（CC 做不了）
     ***REMOVED*** 2. 用户在 OpenRouter dashboard rotate key（CC 做不了）
     ***REMOVED*** 3. 用户更新本地 ~/.env + VPS .env
     ***REMOVED*** 4. CC 执行 scrub:
...
3. **PUBLIC flip**（最后一步，**真不可逆**）
   ...
   - [ ] LFS migrated
   - [ ] README 外部友好版生效（PR ***REMOVED***88 已合）
   - [ ] LICENSE / CONTRIBUTING / CODE_OF_CONDUCT / GOVERNANCE 都在（PUBLIC_READINESS_CHECKLIST 已 ✅）
   - [ ] 最后 prod 200 verify

---

***REMOVED******REMOVED*** 4. 仍未做（user-blocked, 不在本 session scope）
...
| 文件 | 用途 |
|---|---|
| `docs/sessions/HANDOFF.md` | 永久 entry point（应该更新指向 session ***REMOVED***8 状态） |
| **本文件** `SESSION-9-HANDOFF.md` | Session ***REMOVED***9 起手快速摘要 |
...
***REMOVED******REMOVED*** 6. 必备 quirks（继承 + 新加）

继承 session ***REMOVED***7：见 `SESSION-8-HANDOFF.md` §4。

新加（session ***REMOVED***8）:
...
***REMOVED******REMOVED*** 7. Session ***REMOVED***9 推荐起手

***REMOVED******REMOVED******REMOVED*** Option A — Wave 3 destructive ops 完成（前提 user rotated keys）
...
***REMOVED******REMOVED******REMOVED*** Option B — Plausible 数据回看 + tier-aware ask routing
...
***REMOVED******REMOVED******REMOVED*** Option C — Adversarial pre-registration paper（unified paper）
- CVE FAIL + NYC FDNY INCONCLUSIVE + WSB PARTIAL + Phase Detector NULL 四连击合写
- 这是 anti-p-hacking methodology paper 的史诗素材

***REMOVED******REMOVED******REMOVED*** Option D — Discoveries CLS font-fix（W1-B 留下的 fix recipe）
...
Session ***REMOVED***8 用户授权 "GitHub/VPS 全部做"，但 CC safety guard 在含 actual key 的 file 写入处拦下。两个 key 必须先 rotate（CC 做不了，必须 user dashboard 操作），然后 CC 可一键完成 scrub + force-push + LFS + PUBLIC flip。
...
***REMOVED******REMOVED*** 9. Session ***REMOVED***8 stats
