***REMOVED*** Session ***REMOVED***5 Starter — 交接文档

> Session ***REMOVED***4 完成时间：2026-05-14 09:18 CST
> Main HEAD：`b75c5b0`
> 累计 PRs (session ***REMOVED***4)：7 个 merged (***REMOVED***46-52)
> 用户身份：达达 (dada8899)，repo PUBLIC
>
> Session ***REMOVED***5 起手第一件事：读本文件，运行 §0 起手扫描，然后按 §3 优先级队列推进。
...
git log --oneline -5         ***REMOVED*** HEAD 应该是 b75c5b0 或更新
gh auth status                ***REMOVED*** 必须 ✓ logged in dada8899
ls .env                       ***REMOVED*** 本地 DeepSeek key 在这里（gitignored）
```
...
***REMOVED******REMOVED******REMOVED*** 部署命令（如有变更）

```bash
***REMOVED*** Structural 静态前端 + 后端
...
***REMOVED*** Phase Detector Next.js
...
***REMOVED******REMOVED*** §1 Session ***REMOVED***4 落地清单（不要重复做）

7 个 PR 全部 merged 到 main + 部署上线 + 实测 verified：

| PR | 主题 | Lines |
|---|---|---|
| ***REMOVED***46 | W6-D abstract rewrite + CITATION rebrand + README placeholder | +51/-16 |
| ***REMOVED***47 | 全站 copy sweep — V1/V2/V3/V4 sprawl 全清，outcome-first 重写 | +454/-519 |
| ***REMOVED***48 | History sidebar — Structural (vanilla JS, 327 LOC) | +456/-2 |
| ***REMOVED***49 | Phase Detector SearchHero + parse-query.ts NLU | +679/-8 |
| ***REMOVED***50 | History sidebar — Phase Detector (TSX, 190 LOC) | +234/-2 |
| ***REMOVED***51 | Cross-link symmetry polish (主站 → Structural ↗ + 姐妹产品) | +17/-2 |
| ***REMOVED***52 | 关键修复：Phase filter enum + /start-here 404 + 研究报告卡片删 + cross-link 域名 + analyze breadcrumb + 流式 typewriter preview | +752/-477 |
...
***REMOVED******REMOVED******REMOVED*** Multi-dimensional 打分

总分从 **37.8% → 84.4%**（+47 pts）。9 个维度详细见 session ***REMOVED***4 末汇报。新增维度"流式生成体验" 2 → 7。

---

***REMOVED******REMOVED*** §2 Defer 到 session ***REMOVED***5 的事

按重要性排序：

***REMOVED******REMOVED******REMOVED*** P0 — 关键剩余（建议 session ***REMOVED***5 起手做）
...
1. **TL;DR card + analyze 页 section 重排**（user 强需求 + 显眼收益）
   ...
   - 评估：1.5-2h，纯前端
   - 来源：session ***REMOVED***4 Agent C deep-dive 报告
...
2. **/synthesize endpoint SSE 改造**
   ...
   - 评估：后端 1.5-2h + 前端 1h
   - 来源：session ***REMOVED***4 Agent C "synthesize 全 blocking" 发现

***REMOVED******REMOVED******REMOVED*** P1 — 显眼但可缓
...
3. **Shared CSS tokens 5-var 集中化 + GlossaryTooltip 组件**
   ...
   - 评估：3-4h（design + 双栈实现）
   - 来源：session ***REMOVED***4 UX designer agent 推荐
...
***REMOVED******REMOVED******REMOVED*** P2 — 长期但已 staged
...
6. **PyPI publish — soc-pipeline + guarded-llm**
   ...
   - 等 user 拿 `PYPI_TOKEN` 给 session ***REMOVED***5
...
***REMOVED******REMOVED*** §3 Session ***REMOVED***5 起手推荐顺序

```
1. §0 起手扫描（git pull / gh auth / .env / VPS DeepSeek key）
2. 用户确认 session ***REMOVED***5 是否 PYPI + Zenodo 发布优先 (P2 ***REMOVED***6-***REMOVED***9)
   还是 UX 第二轮 (P0 ***REMOVED***1-***REMOVED***2 + P1 ***REMOVED***3-***REMOVED***5) 优先

如果用户选 UX 第二轮：
  3a. TL;DR card + section 重排（P0 ***REMOVED***1）
  3b. /synthesize SSE 改造（P0 ***REMOVED***2）
  3c. Shared CSS tokens + GlossaryTooltip（P1 ***REMOVED***3）
...
***REMOVED******REMOVED*** §4 项目结构速查

```
~/Projects/structural-isomorphism/
├── web/
│   ├── frontend/          ***REMOVED*** Structural 静态 HTML+JS (13 页)
│   │   ├── index.html     ***REMOVED*** 首页（hero + examples + waitlist + sister-product callout）
│   │   ├── classes.html   ***REMOVED*** /classes 共享模式（user 认为这是"真首页"）
│   │   ├── search.html, analyze.html, discoveries.html ...
│   │   ├── start-here.html  ***REMOVED*** /start-here 长文（PR-7 修复了 routing）
│   │   └── assets/
│   │       ├── js/        ***REMOVED*** i18n / utils / search / analyze / history-sidebar / ...
│   │       └── css/
│   ├── phase-detector/    ***REMOVED*** Phase Detector Next.js
...
│       ├── main.py        ***REMOVED*** 主路由 (PR-7 新增 /start-here)
...
│   ├── arxiv-drafts/2026-05-13/01..04_*.md  ***REMOVED*** 4 solo C2
│   └── c4-reject-aware-pipeline-2026-05-13.md
└── docs/sessions/
    ├── HANDOFF.md
    ├── SESSION-4-STARTER.md  ***REMOVED*** session ***REMOVED***3 末交给 ***REMOVED***4 的清单
    └── SESSION-5-STARTER.md  ***REMOVED*** 本文件
```

---

***REMOVED******REMOVED*** §5 不可逆操作清单（注意！）

session ***REMOVED***5 任何动作都要先确认：
...
| **git-filter-repo + force-push** | ❌ | 重写历史不可逆（session ***REMOVED***4 已决策 Path A：rotate key only，不 scrub） |
| **delete repo / branch on GitHub** | ❌ | 需用户授权 |

---

***REMOVED******REMOVED*** §6 已知 quirks
...
***REMOVED******REMOVED*** §7 Session ***REMOVED***5 起手 prompt 模板

如果用户想换 prompt 直接告诉 session ***REMOVED***5 Claude：
...
**Session ***REMOVED***4 结束**。HEAD `b75c5b0`，7 PRs，UX 总分 37.8% → 84.4%（+47 pts）。
