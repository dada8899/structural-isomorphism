***REMOVED*** Session ***REMOVED***10 起手交接文档

> 上 session (***REMOVED***9) 完成于 2026-05-14 ~夜 CST
> Main HEAD: `0681790` (W4 PR merges 将进一步推进)
> Session ***REMOVED***9 共 15 PR squash-merged (***REMOVED***97-***REMOVED***111), 4 wave 并行 sub-agent 模式
...
git log --oneline -20                 ***REMOVED*** HEAD = 0681790 或更新（W4 merge 之后）
gh auth status                        ***REMOVED*** ✓ dada8899

***REMOVED*** Tests
PYTHONPATH=. .venv/bin/python -m pytest web/backend/tests/ -q   ***REMOVED*** 期望 48 pass
PYTHONPATH=. .venv/bin/python -m pytest web/tests/e2e/ -q       ***REMOVED*** 期望 11 pass
...
***REMOVED*** Worktree cleanup（session ***REMOVED***9 用了大量 /tmp/structural-w*）
git worktree list
git worktree prune
```
...
***REMOVED******REMOVED*** 1. Session ***REMOVED***9 已交付（15-19 PRs，4 waves，全部 squash-merged）

***REMOVED******REMOVED******REMOVED*** Wave 1 — 5 agents 并行（PRs ***REMOVED***97-***REMOVED***101，merged 2026-05-14 15:25-26 UTC）

| PR | Agent | 交付 | Metric |
|---|---|---|---|
| *****REMOVED***97** | W1-A | Phase Detector `/about` 加 founder bio + contact section | 单页 polish, 信任信号 |
| *****REMOVED***98** | W1-B | 数据量统一: README 500 → 100（产品 prod 实际 100, 500 是 backtest universe）| 跨 surface 信任修复 |
| *****REMOVED***99** | W1-C | `/classes` 默认 filter 改为 all (23 classes), 不再只露 8 个 manual-curated | discoverability +185% |
| *****REMOVED***100** | W1-D | `/phenomenon/{id}` query-param deep-link 修 + `/analyze` alias 清理 | SEO friendly + bug fix |
| *****REMOVED***101** | W1-E | discoveries 页 self-host fonts → CLS 0 (was 0.219 from font-swap) | CLS fix 1/3 (phase + homepage 后续) |

***REMOVED******REMOVED******REMOVED*** Wave 2 — 5 agents 并行（PRs ***REMOVED***102-***REMOVED***106，merged 2026-05-14 17:01-17 UTC）

| PR | Agent | 交付 | Metric |
|---|---|---|---|
| *****REMOVED***102** | W2-A | beta 高意图点位加 newsletter signup（hero CTA + footer + /paper-bottom）| Plausible 转化点位 +3 |
| *****REMOVED***103** | W2-B | phase-detector hero 加 NULL backtest transparency banner（trust signal）| 反 p-hacking 故事承上启下 |
| *****REMOVED***104** | W2-C | **anti-p-hacking adversarial pre-registration unified paper** (358 行 6445 字)| arXiv-ready, draft v1 |
| *****REMOVED***105** | W2-D | `/api/ask` 加 8 case backend tests + history_db sqlite3 deprecation fix | 22 → 30 → 38 backend tests |
| *****REMOVED***106** | W2-F | **deploy 防灾 infra**: `scripts/restore-models.sh` + `scripts/deploy-vps.sh` + `docs/deployment/DEPLOY.md` + `nginx-structural-beta.conf` | rsync --delete 灾难永不复发 |

***REMOVED******REMOVED******REMOVED*** Wave 3 — 5 agents 并行（PRs ***REMOVED***107-***REMOVED***111，merged 2026-05-14 16:48 UTC）

| PR | Agent | 交付 | Metric |
|---|---|---|---|
| *****REMOVED***107** | W3-A | **真实 e2e** (Playwright `test_real_user_flows.py`): 4 flow × 2 viewport · 撞 3 P0 (CLS / card-click / newsletter-state) | docs/session-9-real-e2e-2026-05-15.md |
| *****REMOVED***108** | W3-B | phase-detector `next/font` self-host (Google Fonts cross-origin 去掉) | build artifact in `.next/static/media/` |
| *****REMOVED***109** | W3-C | beta + phase 首屏加 "explore cards + recently flipped" first-fold | landing engagement 信号 |
| *****REMOVED***110** | W3-D | README badges (arXiv/citation/tests/methodology) + `robots.txt` + `sitemap.xml` | SEO + 外部友好 |
| *****REMOVED***111** | W3-E | **beta backend auto-deploy CI** (`.github/workflows/deploy-beta-backend.yml`)| push main → VPS deploy |

***REMOVED******REMOVED******REMOVED*** Wave 4 — 5 agents 并行（在 flight，session 末 squash-merge）
...
| **W4-E** | **本文件** + session ***REMOVED***9 closeout 文档 | 完成中 |

> 注: W4 实际 merge 数取决于 sub-agent 完成度。预计 4-5 PR 进 main, session ***REMOVED***10 起手 git pull 可验证。

---

***REMOVED******REMOVED*** 2. Session ***REMOVED***9 metrics

- **15-19 PRs merged** (***REMOVED***97-***REMOVED***111+W4), 4 waves of 4-5 sub-agents
...
***REMOVED******REMOVED*** 3. ⚠️ 仍 user-blocked items (继承 session ***REMOVED***8 §3)
...
| **model v2 真权重恢复** | 现用 base fallback `shibing624/text2vec-base-chinese`, search precision 降级。原 finetune script 在 `training/`? | 需要 session ***REMOVED***10 + 1 工作日 |
...
| 文件 | 用途 |
|---|---|
| `docs/sessions/HANDOFF.md` | 永久 entry point（应该更新指向 session ***REMOVED***9 状态）|
| **本文件** `SESSION-10-HANDOFF.md` | Session ***REMOVED***10 起手快速摘要 |
| `docs/sessions/SESSION-9-HANDOFF.md` | 上 session 起手交接（session ***REMOVED***8 closeout）|
...
| `paper/cve-preregistration-fail-2026-05-14.md` | CVE FAIL paper 4610 词 arXiv-ready (session ***REMOVED***8 继承) |
...
***REMOVED******REMOVED*** 5. 必备 quirks（session ***REMOVED***9 新加，继承 ***REMOVED***8 §6）

继承 session ***REMOVED***8 §6 全部 quirks（CC safety guard / worktree cleanup / gh pr merge --auto / nginx exact-match / arctic_shift / pnpm-vs-npm）

新加（session ***REMOVED***9）:
...
8. **explicit `git add <file>` 是铁律** — 永远禁 `-A` / `.` / `commit -a`。每个 sub-agent prompt 都强调 + verify `git diff --cached --name-only`。session ***REMOVED***9 4 wave 17+ sub-agent, **0 commit-boundary 违反**。
9. **真实 dogfood > 单测全过** — W3-A + W4-D 用浏览器和真 LLM 跑一遍, 单测 + e2e 全过的代码暴露 3 P0。任何含逻辑改动必须三层验证（unit + integration + real-env e2e）。

---

***REMOVED******REMOVED*** 6. Session ***REMOVED***10 推荐起手（4 options）

***REMOVED******REMOVED******REMOVED*** Option A — 用户解锁 Wave 3 destructive ops（最高 ROI）
...
***REMOVED******REMOVED******REMOVED*** Option B — model v2 真权重恢复（1 工作日工作量）
...
***REMOVED******REMOVED******REMOVED*** Option C — `VPS_BETA_DEPLOY_KEY` 配 + 跑通 beta-backend CI
...
***REMOVED******REMOVED******REMOVED*** Option D — Wave 5 继续 polish（CC 独立, 无 user 介入）

session ***REMOVED***7 dogfooding 留 12 P1 backlog（详 `docs/sessions/SESSION-7-end.md` + `docs/sessions/milestones/`）:
...
CC 可独立做 Wave 5, 类似 ***REMOVED***9 流程 4 wave × 5 agent。

---

***REMOVED******REMOVED*** 7. 用户当下需求理解

Session ***REMOVED***9 用户授权 "auto mode 跑 ~3-4 小时, 做完一波 self-audit 找下一波 9/10 标准"。CC 已 4 wave 在跑（W1-W4 = 17+ agent, 15-19 PR）。

session ***REMOVED***10 起手第一句话建议:

> **"用户解锁 Wave 3 destructive ops + 给 arXiv/PyPI/Zenodo token, CC 一键打包发布"**

原因:
- session ***REMOVED***9 已把所有 OSS-ready 前置工作做完（防灾 infra / SEO / e2e / paper / packages）
...
***REMOVED******REMOVED*** 8. Session ***REMOVED***9 stats summary

- **15-19 PR merged** in 1 session (***REMOVED***97-***REMOVED***111 + W4 in flight)
...
***REMOVED******REMOVED*** Wave 5 closeout addendum (2026-05-15 late)

W4-E (***REMOVED***116) 写 handoff 时 Wave 4 还未全部 merge。Wave 5（5 个并行 sub-agent，含本 W5-E final closeout）补充实际收尾状态。

***REMOVED******REMOVED******REMOVED*** Wave 5 PR queue (待主 session merge)
...
***REMOVED******REMOVED******REMOVED*** 实际累计 metrics (Wave 1-5 全收尾)
...
***REMOVED******REMOVED******REMOVED*** 每环节 9/10 状态确认 (2026-05-15)
...
***REMOVED******REMOVED******REMOVED*** session ***REMOVED***10 推荐起手 update (3 个 user-input only steps)

session ***REMOVED***10 起手 CC 等用户做完以下 3 个 manual step 即可一键发布：
...
3 步完成后 session ***REMOVED***10 可立即进入 Wave 6 = **Alpha 用户招募 + 真实科研用户 dogfood**。

***REMOVED******REMOVED******REMOVED*** 推翻 W4-E 原 handoff 的一处建议

W4-E 建议 session ***REMOVED***10 第一句"用户解锁 Wave 3 destructive ops + 给 token"，Wave 5 closeout 后**升级为**：
