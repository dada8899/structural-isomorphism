...
| 维度 | 当前值 |
|---|---|
| 容器背景 | `--brand-paper-card` = `#FFFFFF` |
| 页面背景 | `--brand-paper` = `#F5F5F4`（暖灰） |
| 边框 | `1.5px solid --brand-line #E4E4E7` |
| 圆角 | `20px`（移动 480 → `14px`） |
| 内边距 | `18px 20px 12px` |
| 阴影 | `0 1px 2px rgba(24,24,27,.04), 0 8px 24px rgba(24,24,27,.08)` |
| 输入字号 | `18px` / 行高 1.55 / `min-height 56px` |
| placeholder 色 | `--brand-muted` = `#71717A` |
| placeholder 文案 | 「问点复杂的——它可能在另一个学科已经被解过」 |
| focus 态 | 边框变 `--brand-accent #2563EB` + `0 0 0 4px rgba(37,99,235,.12)` 蓝光环 |
| 提交按钮 | 44×44 圆形，`--brand-accent #2563EB` 实心蓝底白箭头；disabled 时灰底 |
| footer | 顶部 1px 分隔线，左侧 ⌘+Enter 提示（默认 opacity:0），右侧字数计数 + 提交按钮 |

---

## 1. 诊断：「别扭」的具体原因（按可能性排序）

### 主因 ① 提交按钮的「实心正蓝圆球」是最大违和源 —— 不是边框，是按钮

用户怀疑「蓝色」，方向对，但**罪魁不是 focus 边框，是右下角那个 44px 的实心蓝色圆形提交按钮**。

- 色值 `#2563EB`（Tailwind blue-600）：HSL ≈ `217°, 83%, 53%`。**饱和度 83% 在一个白底 + `#F5F5F4` 暖灰 + 衬线标题的克制版面里是异类**——整页其余颜色都是中性灰阶（`#18181B` / `#52525B` / `#71717A` / `#E4E4E7`），只有这一个高饱和正蓝。
...
### 主因 ② 容器是「纯白卡片」浮在「暖灰背景」上，色温打架

- 页面背景 `#F5F5F4` 是**暖中性灰**（带一点点黄），搜索框背景 `#FFFFFF` 是**纯白**。两者并排，白卡片会显得「冷、生、像贴上去的」。
...
### 次因 ③ focus 态蓝光环 `4px` 偏厚 + 正蓝，二次强化违和

- focus 时 `0 0 0 4px rgba(37,99,235,.12)` —— 4px 光环偏厚（Apple 风格通常 3px 且更淡），且仍是 `#2563EB` 正蓝。这本身不算大问题（focus ring 用品牌色是合理的），但它和主因①的蓝按钮**同框出现**，把「整页唯一的高饱和蓝」从一个点变成一圈，违和被放大。
- 它只在 focus 时出现，所以也属于「静态看不出、一交互才别扭」。

### 次因 ④ footer 分隔线把一个框「劈成两半」
...
.ask-searchbox__submit {
  /* was: background: var(--brand-accent);  #2563EB */
  background: var(--brand-ink);     /* #18181B 近黑，安静、与版面同族 */
  color: #fff;
}
.ask-searchbox__submit:hover {
  background: #000;                  /* 或保留 --brand-accent 作为 hover 微反馈 */
}

/* 2. focus 光环：4px → 3px，并降一档不透明度 */
.ask-searchbox:focus-within {
  border-color: var(--brand-ink-soft);            /* #52525B 中性，不用正蓝 */
...
### 方案 B — 推荐基线：保留品牌蓝，但换「形态」+ 收口卡片（改 4 处）
...
.ask-searchbox:focus-within {
  border-color: var(--brand-line-strong);          /* #D4D4D8，安静地变实 */
...
└──────────────────────────────┘     └──────────────────────────────┘
   ...
   ●= #2563EB 实心圆，整页最跳            [↗]= 浅蓝底蓝箭头，安静；hover才实心
```
