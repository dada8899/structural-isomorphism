...
***REMOVED******REMOVED******REMOVED*** P0-1 「我的报告」(/reports) 永远是空的，功能完全不可用
**复现**：
...
***REMOVED******REMOVED******REMOVED*** P0-2 已保存/已分享报告的查看页排版严重降级，显示原始英文字段名
**复现**：
...
***REMOVED******REMOVED******REMOVED*** P0-3 `/api/report/{id}` 对真实存在的报告返回 404
**复现**：
...
***REMOVED******REMOVED******REMOVED*** P0-4 信息架构分裂：部署的首页是「问答 UI」，但「搜索流」挂在一套无入口的旧首页
**复现 / 证据**：
- 生产 `index.html` 用的是 `***REMOVED***ask-form` / `.ask-searchbox__input`，加载 `ask.js`，提交到 `POST /api/ask/stream`（Perplexity 式问答）。
- 但 `home.js` 用的是 `***REMOVED***search-form` / `.searchbox__input`，跳转 `/search?q=`——这是另一套首页实现，**生产首页根本没加载它**。
...
***REMOVED******REMOVED*** P1 — 应修（影响体验，不一定阻断）

***REMOVED******REMOVED******REMOVED*** P1-1 主导航没有「我的报告」入口
...
***REMOVED******REMOVED******REMOVED*** P1-2 站内导航在不同页面严重不一致
...
***REMOVED******REMOVED******REMOVED*** P1-3 report.html / reports.html 的 Plausible 指向错误域名
...
***REMOVED******REMOVED******REMOVED*** P1-4 report.html 的 KaTeX 走 CDN，与全站自托管策略冲突
...
***REMOVED******REMOVED******REMOVED*** P1-5 `/report/<id>` 模式下不渲染分享栏
...
***REMOVED******REMOVED******REMOVED*** P1-6 报告生成首字节延迟可达 30s+，加载文案承诺「30–60s」但实测整篇 >180s
...
***REMOVED******REMOVED******REMOVED*** P1-7 报告生成流偶发零字节返回（流可靠性）
...
***REMOVED******REMOVED******REMOVED*** P1-8 首页问答流里没有「查看/管理已存报告」的任何线索
...
***REMOVED******REMOVED******REMOVED*** P1-9 移动端缩放被锁：多页面缺 maximum-scale 但 index/learn/thank-you 设了 maximum-scale=5.0
...
***REMOVED******REMOVED******REMOVED*** P2-1 `/api/search` 仅支持 POST，GET 返回 405
...
***REMOVED******REMOVED******REMOVED*** P2-2 报告分享页 `report.html` 的英文/中文混用
...
***REMOVED******REMOVED******REMOVED*** P2-3 report.html 错误文案用了英文标点
`report.html`：「这份报告可能已被删除,或链接已失效。」「分享前请确认.」——用了半角逗号/句号。中文排版应用全角「，」「。」。

***REMOVED******REMOVED******REMOVED*** P2-4 `analyze.js` 在缺 `b_id` 时静默重定向回首页
...
***REMOVED******REMOVED******REMOVED*** P2-8 `analyze.html` 默认 `persist=1` 但用户无知情、无开关
...
***REMOVED******REMOVED*** 走查覆盖清单（已实测/已走读）
...
| 收藏 | ✅ | localStorage，badge、首页 ***REMOVED***home-favorites |
| 历史侧边栏 | ✅ | 链到 /search |
