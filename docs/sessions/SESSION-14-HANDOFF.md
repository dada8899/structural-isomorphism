...
> 2026-05-20。接 session #13(`SESSION-13-HANDOFF.md`)。供下个 session 起手用。
...
| main HEAD | 同 session #13(三个 PR 都未 merge) |
| PR #222 | session #13 — CI 修复 — CI 全绿,待 merge |
| PR #223 | session #13 — 首页搜索框方案 A — 待 merge |
| **PR #224** | **session #14 — M1.2 fix1/fix4 + M1.3 拒答门 — 47 测试全绿,待 merge** |

## 2. Session #14 做了什么

PR #224(4 commits)。M1.2 / M1.3 体验硬伤修复。
...
| `297ca77` | 归档 session #13 文档(HANDOFF / PRODUCT-DIRECTION / M1-experience-fix-research) |
...
- merge PR **#222** + **#223** + **#224**:GitHub 网页 "Squash and merge"
...
- `scripts/train_v2.py` 工作树有未提交改动(session #13 起手即存在,#14 按 commit 边界铁律未动)— 需确认归属
- agent 隔离 worktree `.claude/worktrees/agent-a3e2f585dec5d670b` 残留(同 session #13)
- 三层测试 layer 3(真实环境 e2e):#224 的拒答路径零 OpenRouter $,合并部署后随 §4 dogfood 复跑即可低成本完成
