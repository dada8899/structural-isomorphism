...
> Session #11 完结于 2026-05-20，本文档供下个 session（本人换设备或新 CC session）起手用。
...
### CI workflow 状态（最新 push 后）

| Workflow | Status | 备注 |
|---|---|---|
| types-sync | ✅ | F11 #207 持续绿 |
...
| ci (matrix) | ⏳ ubuntu/macOS 应绿 | windows-latest 已 exclude (PR #217); 其他 RUNNING 待验 |
...
## 2. Session #11 交付物（21 PR + 多项 infra）

### Wave 1（audit findings 修）— PR #199-#204
- **#199** ci 装 editable packages/* + pyyaml + mkdocstrings
- **#200** FastAPI on_event → lifespan
- **#201** Next.js 14.2.15 → 14.2.35 (CVE-2024-56332 + cache poisoning + mw bypass)
- **#202** tier-aware rate limit ContextVar + 7 LLM-expensive endpoints
- **#203** polish: CITATION 0.4.0 + /me redirect + 404 zh + ESLint config + 加载更多
- **#204** OpenAPI schema sync + 9 response_model

### Wave 2（CI deepening）— PR #205-#211
- **#205** storybook fixtures schema align (UniversalityEvidenceSystem)
- **#206** perf regex v1（leading char + bare /）
- **#207** TS types regen 跟 #204 同步
- **#208** deploy-vps.sh `export CI=true`（pnpm TTY bypass）
- **#209** pnpm-lock.yaml refresh after #203 eslint
- **#210** coverage 装 pyjwt+structlog+openai+sklearn + continue-on-collection-errors
- **#211** mkdocs pin <1.0 + drop --strict（临时）

### Wave 3（真根因迭代）— PR #212-#214
- **#212** perf 真根因 = lint blocks build + pipefail + actionable error dump
- **#213** sanity PYTHONPATH/conftest（v4/lib append + dotted path shim imports）
- **#214** sanity 缺 sklearn 装

### Wave 4（公开 + 清尾）— PR #217-#221
- **#217** ci matrix exclude windows-latest（3 platform issues: cp1252 编码 + subprocess paths）
- **#218** PyPI publish workflow + `docs/deployment/PYPI_PUBLISH.md`（tag-trigger + token/OIDC dual-auth）
- **#219** mkdocs --strict 重启 + 4 cp + 7 broken links 修
- **#220** ESLint `react/no-unescaped-entities` 重启 + 22 CJK 引号修
- **#221** coverage workflow `-p` flag（5 处）— combine bug 真根因
...
### B7 历史 LLM key 暴露 — repo 已 PUBLIC 5+ 天
...
```bash
# 1. Vendor dashboard 立即 rotate
# OpenRouter: https://openrouter.ai/settings/keys → revoke + create new
# DeepSeek: https://platform.deepseek.com/api_keys → revoke + create new

# 2. 验证旧 key 已 dead (curl 应得 401)
curl -X POST https://openrouter.ai/api/v1/chat/completions \
  -H "Authorization: Bearer sk-or-v1-af9ae735..." \
  -d '{"model":"openai/gpt-4o-mini","messages":[{"role":"user","content":"test"}]}' 
# 期望 401

# 3. 更新 VPS systemd env / .env 文件，重启服务
ssh vps "systemctl restart beta-structural-backend phase-detector-web"

# 4. 删除 audit branch（含 plaintext key 的额外暴露载体）
...
- **Route B (filter-repo + force-push)**：彻底删历史。会 break #215/#216 等 open PR，fork 残留无法清。耗时 30 分钟。命令在 `docs/security/2026-05-20-history-key-audit.md`（audit branch 那 252 行）。

CC 推荐 Route A — 既然 key rotate 了，明文历史 risk = 0，不值得 force-push 的成本。

---

## 4. 仍 user-input only blocker（非紧急）

| Item | 需要 |
|---|---|
| PyPI 实际 publish | `PYPI_API_TOKEN` GH secret + git tag 触发自动（workflow #218 已建） |
...
### Option A — 1 小时 OSS launch（最高 ROI）
...
- **mkdocstrings 1.x 真升级**（现 pin <1.0 是 #211 临时方案）

### Option C — Wave 16+ feature

- Model v3 训练（多语言 + 大 KB + LoRA refresh）— 需 GPU
- session #7 P1 backlog 残（rich-text annotation / citation density viz / multi-author collab）
...
| File | 用途 |
|---|---|
| **本文件** | session #12 起手快速摘要 |
| `docs/sessions/session-11-close-out.md` | session #11 详细总结（21 PR breakdown） |
| `docs/sessions/SESSION-11-HANDOFF.md` | session #11 起手时的交接（session #10 末态） |
...
git pull origin main                                          # HEAD 应 = 9febcc9
git log --oneline -10                                         # 看 wave 4 PR
cat docs/sessions/SESSION-12-HANDOFF.md                       # 本文件
gh run list --branch main --limit 12                          # 看 CI 状态
```
...
> Session #11 落幕。21 PR + 1 release + 全程 prod 4 域名 200 + 0 commit-boundary 违反 + 0 outage。
