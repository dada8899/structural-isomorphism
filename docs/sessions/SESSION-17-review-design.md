...
- **改法**：加一个标准移动汉堡菜单。`.site-header` 右侧放 44×44px 的菜单按钮（iOS 最小可点区域），点击展开全屏或抽屉式 nav，复用桌面那 8 个链接。抽屉背景 `--brand-paper-card #FFF`，链接 `font-size:18px`、行高 56px、`--brand-line` 分隔线。不要保留「nav 直接消失」这个方案。
...
### P1-3 报错卡片用蓝色聚焦边框
- **位置**：`report.html` 错误卡片。
- **现象**：「报告不可用」卡片用了亮蓝色 (`--brand-accent #2563EB`) 的粗边框 + 蓝色外发光，视觉上像「被选中的输入框」，与「错误」语义冲突。
- **改法**：错误卡片用中性边框 `--brand-line #E4E4E7` 1px，无外发光；或用 `--danger-border #FECACA` + `--danger-surface #FEF2F2` 的克制错误样式。不要用 accent 蓝。

### P1-4 classes 页信息密度过高、缺留白节奏
...
### P1-7 discoveries hero 与上方订阅卡片层级倒挂
...
### P1-9 术语不统一：「我的收藏」vs「我的报告」vs「已保存的报告」
