***REMOVED*** Data Versioning Strategy
...
***REMOVED******REMOVED*** 哪些文件进 LFS

***REMOVED******REMOVED******REMOVED*** 通用 pattern（按扩展名）
...
***REMOVED******REMOVED******REMOVED*** Path-specific pattern（按目录）
...
***REMOVED******REMOVED******REMOVED*** 明确**不**进 LFS
...
***REMOVED******REMOVED*** 新人 onboarding

clone repo 后做一次：

```bash
***REMOVED*** 1. 装 git-lfs（如果还没装）
brew install git-lfs        ***REMOVED*** macOS
***REMOVED*** 或 apt-get install git-lfs  ***REMOVED*** Linux

***REMOVED*** 2. 初始化 lfs hooks（每个 fresh clone 都要做一次）
git lfs install

***REMOVED*** 3. 拉取 LFS 对象（如果 repo 里已有 LFS 文件）
git lfs pull
```

未做 step 3 时，LFS 文件会是 ~130 字节的 pointer 文本（不是真数据），脚本读会报奇怪错误。

***REMOVED******REMOVED*** 加新大文件的流程

```bash
***REMOVED*** 1. 如果新文件不在 .gitattributes 已有 pattern 内，先 track 它
git lfs track "v4/validation/new-domain/raw/*.csv"
***REMOVED*** 自动 append 到 .gitattributes

***REMOVED*** 2. 把 .gitattributes 的更改 commit（必须先 commit attributes，再 commit 文件）
...
***REMOVED*** 4. push（lfs 对象会自动推到 lfs server）
git push
```

***REMOVED******REMOVED*** 验证 LFS 是否生效

```bash
***REMOVED*** 看哪些 pattern 被 track
git lfs track

***REMOVED*** 看本地有哪些 LFS 对象
git lfs ls-files

***REMOVED*** 检查某个文件是否走 LFS
git check-attr filter <path/to/file>
***REMOVED*** 输出含 `filter: lfs` 则是 LFS
```

***REMOVED******REMOVED*** 已知风险与约束
