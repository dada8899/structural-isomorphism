***REMOVED*** MODEL_RECOVERY.md — structural-v2 权重恢复手册

> 2026-05-14 W1 prod disaster 后撞出来的事实：`models/structural-v2/` 的"真"finetuned 权重**目前不存在任何可恢复来源**。本文档钉死现状 + 3 条恢复路径 + 防灾建议。

***REMOVED******REMOVED*** 现状（session ***REMOVED***9 W5-C audit, 2026-05-14）

***REMOVED******REMOVED******REMOVED*** prod 在用什么 model
...
- **README 自曝**：`/root/Projects/structural-isomorphism/models/structural-v2/README.md` 第一行 `***REMOVED*** shibing624/text2vec-base-chinese`，原样从 base model README 复制
- **文件大小**：409 MB safetensors（与 base BERT-base 参数量一致，无 finetune layer 增量）

***REMOVED******REMOVED******REMOVED*** W1 是怎么丢的
...
***REMOVED******REMOVED******REMOVED*** W4-D dogfood 实锤的精度问题
...
***REMOVED******REMOVED*** 3 条恢复策略（`scripts/recover-model-v2.sh`）

***REMOVED******REMOVED******REMOVED*** Strategy 1: HF Hub snapshot_download
...
***REMOVED******REMOVED******REMOVED*** Strategy 2: 本地 re-finetune（**唯一目前可行**）
...
***REMOVED******REMOVED******REMOVED*** Strategy 3: base fallback（**当前 prod 状态**）
...
***REMOVED******REMOVED*** 防灾建议（钉死，未来不应再撞）

***REMOVED******REMOVED******REMOVED*** B1. HF Hub 作为 canonical source
...
***REMOVED******REMOVED******REMOVED*** B2. deploy 永不带 `--delete`

- `docs/deployment/DEPLOY.md` 已钉死（W2-F session ***REMOVED***103）
- `scripts/deploy-vps.sh` 默认 `rsync -avu`（不带 `--delete`）
- 任何含 `--delete` 的 deploy 必须显式 user 授权 + dry-run 预审

***REMOVED******REMOVED******REMOVED*** B3. 训练 artifact 单独备份
...
***REMOVED******REMOVED******REMOVED*** B4. README 必标真实身份
...
***REMOVED******REMOVED*** 本 session（***REMOVED***9 W5-C）做了什么
...
***REMOVED******REMOVED*** 下个 session 应该做什么
