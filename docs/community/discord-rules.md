...
`CODE_OF_CONDUCT.md` at the repo root — when in doubt, the COC governs.
...
sneering, dismissiveness, and tone-policing of newcomers are not. Critique
ideas, not people. When you're wrong, say so — it's the most respected
behavior in this server.

**2. No harassment, in any form.**
Sexist, racist, ableist, homophobic, transphobic, or otherwise bigoted
language earns an instant timeout. Persistent or severe cases earn a ban.
This includes "ironic" usage. See `coc-enforcement-playbook.md` for the
...
and bot-pumped self-promotion are all spam. We respect actual LLM-assisted
work; we don't respect lazy LLM-pumped engagement farming. Repeat offenders
get banned.
...
**8. Don't dox, don't leak, don't off-server harass.**
Posting someone's real name, employer, address, or any private info without
consent is an instant permanent ban. Same for organizing off-server
harassment campaigns. We will cooperate with platform-level reporting in
severe cases.
...
1. **Warning** — DM, clarifying the rule and what to do differently
2. **24-hour timeout** — `@muted` for one day
3. **7-day timeout** — `@muted` for one week
4. **Permanent ban** — removed from the server

For severe violations (harassment, doxing, off-server organizing), we skip
straight to tier 3 or 4. Appeals: 14-day window, council vote, BDFL final
call. See the playbook for templates and edge cases.
...
- We will not "both sides" harassment. The COC is not symmetric.
- Maintainers are held to the **same or higher** standard than members. If
  a maintainer violates the COC, the council (not the BDFL alone) handles
  it.
...
on these rules, open a PR on `docs/community/discord-rules.md` — they
