***REMOVED*** API Key Rotation Runbook
...
***REMOVED******REMOVED*** When to rotate
...
***REMOVED******REMOVED*** Step 1 — Provision a new key

***REMOVED******REMOVED******REMOVED*** DeepSeek
...
***REMOVED******REMOVED******REMOVED*** OpenRouter (if used)
...
***REMOVED******REMOVED*** Step 2 — Update local environment

Edit your local `.env` (which is gitignored):

```bash
cd /path/to/structural-isomorphism
cp .env.example .env  ***REMOVED*** if you don't already have one
$EDITOR .env
***REMOVED*** Fill in: DEEPSEEK_API_KEY=<new-key>
...
***REMOVED*** Then restart any services that read the env at startup
ssh root@43.156.233.71 'systemctl restart si-* 2>/dev/null || true'
```

---

***REMOVED******REMOVED*** Step 4 — Revoke the old key
...
***REMOVED******REMOVED******REMOVED*** OpenRouter
...
***REMOVED******REMOVED*** Step 5 — Scrub git history (if key was committed)
...
***REMOVED******REMOVED******REMOVED*** Option A: `git filter-repo` (preferred)

```bash
***REMOVED*** Install once
pip install --user git-filter-repo

***REMOVED*** Back up the repo first
cd /tmp
git clone --mirror git@github.com:dada8899/structural-isomorphism.git si-backup.git

***REMOVED*** Then in your working clone:
cd /path/to/structural-isomorphism
git filter-repo --replace-text <(echo 'sk-OLDKEYHEX==><REDACTED-KEY>')

***REMOVED*** Force-push (DESTROYS shared history; coordinate with all collaborators)
git push --force --all origin
git push --force --tags origin
```

***REMOVED******REMOVED******REMOVED*** Option B: BFG Repo-Cleaner (faster for large repos)
...
***REMOVED******REMOVED******REMOVED*** After force-push
...
***REMOVED******REMOVED*** Step 6 — Audit + lessons
...
***REMOVED******REMOVED*** Preventive checklist (do this once, then forget)
...
```bash
***REMOVED*** One-time pre-commit setup
