***REMOVED*** P1/P2 Visual Polish — beta.structural.bytedance.city
...
***REMOVED******REMOVED*** P1 Fixes

***REMOVED******REMOVED******REMOVED*** 1. Mobile submit button label overflow (`responsive.css:94-108`)
...
***REMOVED******REMOVED******REMOVED*** 2. Home lede WCAG AA contrast (`design-system.css:23`, `home.css:103-121`)
Deepened the global `--text-tertiary` token from `***REMOVED***A1A1AA` (~2.8:1 on `***REMOVED***FAFAF9`) to `***REMOVED***71717A` (~4.6:1). Added `--text-muted: ***REMOVED***A1A1AA` to preserve the lighter tone for decorative use only. Also switched `.home__lede` from `--text-tertiary` to `--text-secondary` as an extra safety margin for body copy. The old light shade is therefore no longer reachable from body copy.

***REMOVED******REMOVED******REMOVED*** 3. `.disc-hero__title` CJK glyph collision (`discoveries.css:46-66`)
...
***REMOVED******REMOVED******REMOVED*** 4. Hero evidence pill (`home.css:132-230`)
...
***REMOVED******REMOVED******REMOVED*** 5. `analyze.html` inline `<style>` block
...
***REMOVED******REMOVED*** P2 Fixes

***REMOVED******REMOVED******REMOVED*** 6. Typography scale unification (`design-system.css:69-73`)
...
***REMOVED******REMOVED******REMOVED*** 7. Hardcoded pixel values
...
- `home.css:1239` — replaced `color: ***REMOVED***d97706` with `var(--warning)`.
- `search.css:528-550` — replaced `***REMOVED***fecaca / ***REMOVED***fef2f2 / ***REMOVED***991b1b / ***REMOVED***b91c1c` with `var(--danger-border|-surface|-strong|-text)`. New tokens added to design-system.css lines 37-41.

***REMOVED******REMOVED******REMOVED*** 8. Card surface flatness (`design-system.css:9`)
Darkened `--bg-primary` from `***REMOVED***FAFAF9` → `***REMOVED***F5F5F4` (Stone-100). White card surfaces (`--bg-tertiary: ***REMOVED***FFFFFF`) now sit on a noticeably cooler background — tonal delta jumps from ~0.3% to ~2%, giving `.how-step`, `.home__daily-card`, `.rec-placeholder`, `.result-card` etc. perceptible lift without needing added shadow everywhere. The existing shadow-sm/md tokens still apply where they were already attached.

***REMOVED******REMOVED******REMOVED*** 9. Footer links uniformity
...
***REMOVED******REMOVED*** New design tokens added
...
***REMOVED******REMOVED*** Skipped / deferred
