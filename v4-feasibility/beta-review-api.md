***REMOVED*** Beta Structural API Review
...
***REMOVED******REMOVED*** Endpoint Inventory
...
***REMOVED******REMOVED*** Bugs

***REMOVED******REMOVED******REMOVED*** P0 — None
No 500s, no crashes, no data corruption observed.

***REMOVED******REMOVED******REMOVED*** P1 — Schema/Behavior Issues
...
***REMOVED******REMOVED******REMOVED*** P2 — Quality / UX
...
6. **Junk-character query `!@***REMOVED***$%^&*()_+`** with rewrite=false returns 5 random results scoring 8.9-9.5 (参照群体剥夺感, 湿壁画的即时性约束, 第三人效果). No score floor — every query produces "matches" even when the input is meaningless.
...
9. **`mapping/stream` query-mode similarity 9.13** for "如何提升团队效率" ↔ 飞机升力与失速 — same normalization bug as P1***REMOVED***1, but additionally the value is meaningless to surface to users (UI shows it as a percentage).
...
***REMOVED******REMOVED*** Validation Behavior (good)
...
***REMOVED******REMOVED*** Recommended Fix Priority

1. Fix the global 404 handler so `/api/*` returns JSON (10 LOC, P1***REMOVED***2).
2. Normalize all `similarity` fields to cosine-in-[0,1] across daily/mapping/analyze (P1***REMOVED***1, P2***REMOVED***9).
3. Move LLM rewrite off the critical path of `/api/search` or switch to Haiku — 15s p95 is the single biggest UX problem (P1***REMOVED***3).
4. Add a score floor on `/api/search` so junk queries return 0 results instead of fake matches (P2***REMOVED***6).
5. Add a synthesize cache keyed by `(query_hash, top_ids)` (P2***REMOVED***7).
6. Precompute `/api/examples` payload at startup (P2***REMOVED***8).

***REMOVED******REMOVED*** Backend File References
