...
***REMOVED*** Create GitHub release v0.3.0 with wheels + dataset runbook as attachments
***REMOVED***
***REMOVED*** Requires: gh CLI authenticated (gh auth status)
***REMOVED*** Usage:
***REMOVED***   bash scripts/release/create_github_release.sh
***REMOVED***   ***REMOVED*** Optional draft mode:
***REMOVED***   DRAFT=1 bash scripts/release/create_github_release.sh
...
***REMOVED*** Verify wheels exist (rebuild if missing)
...
***REMOVED*** Check tag doesn't already exist
if git rev-parse "$VERSION" >/dev/null 2>&1; then
  echo "Tag $VERSION already exists locally. Skipping tag creation."
else
  echo "Creating tag $VERSION on HEAD..."
  git tag -a "$VERSION" -m "structural-isomorphism v0.3.0 — session ***REMOVED***3 mega-sprint + session ***REMOVED***4 F-track release prep"
...
gh release create "$VERSION" \
  --title "structural-isomorphism v0.3.0 — session ***REMOVED***3 mega-sprint" \
