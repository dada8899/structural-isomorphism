...
- §3.13 *Scheffer p-value corrected via block bootstrap.* The original naive p_AR1 = 1.6e-186 and p_Var = 1.9e-127 were numerically meaningless on a serially-correlated daily series with lag-1 autocorrelation ~0.8-0.9. Block bootstrap (l_block = 30 d, n_boot = 1,000) gives **p_block_AR1 = 0.074, p_block_Var = 0.206** — *marginal-to-not-significant*. Verdict revised from "partially confirmed" to "block-bootstrap inconclusive; the original 'p << 1e-120' was a serial-correlation artifact".
- §7.1 *FWER limitations section.* New explicit discussion of multiple-comparisons exposure across 30+ hypothesis tests; Bonferroni and Benjamini-Hochberg sensitivity analysis.
- §8 *Pre-registration for future systems.* New section proposing five adversarial pre-registered tests on systems chosen *before* fitting, with decision rules for class survival.
...
We assemble such a pipeline — Clauset-Shalizi-Newman 2009 MLE power-law fitting with KS-driven $x_\mathrm{min}$ selection, bootstrap CIs, Vuong likelihood ratios against lognormal and exponential, Omori-Utsu temporal stacking, matched-$n$ synthetic null controls, log-binned density estimation, and Bayesian Information Criterion (BIC) model comparison — into one shared Python module (`v4/lib/soc_pipeline.py`, 339 lines, frozen at commit `7ee228c`) and apply it unchanged to **thirteen independent datasets** spanning geology, equity finance, decentralized finance, neuroscience, plasma astrophysics, ecology, banking history, software communities, power grids, highway traffic, and lake biogeochemistry.

Recovered tail exponents fall inside predicted bands across nine SOC and preferential-attachment phases ($\alpha \in [1.08, 3.00]$, full per-system figures in §3). A2-Hysteresis confirms the Preisach first-order signature on NGSIM US-101 traffic with literature-anchored loop-width ratio $1.38 \in [1.25, 1.55]$. A2-Scheffer on Fox River dissolved oxygen was initially reported as $\tau_\mathrm{AR1} = +0.284$ at a naive $p \ll 10^{-120}$, but a 1,000-resample moving block bootstrap ($\ell_\mathrm{block} = 30$ d, addressing the daily series' lag-1 autocorrelation of 0.8-0.9) returns $p_\mathrm{block, AR1} = 0.074$ and $p_\mathrm{block, Var} = 0.206$ — **inconclusive pending cross-site replication**, with the original p-value attributed to a serial-correlation artifact. All four synthetic non-power-law nulls are correctly rejected. Under the finite-size-scaling ansatz $P(s) = s^{-\alpha} f(s/s^*)$, seven systems show shape-normalized functional-form collapse at $r_\mathrm{shape} = 1.11$; BIC prefers power-law + exponential cutoff in 5/7 systems, with lognormal winning 0/7.

A B3 multi-model ensemble taxonomy critic across 21 candidate universality classes (three DeepSeek reviewers, 63 verdicts, 0 parse errors) returned KEEP=5 / REJECT=7 / SPLIT=5 / MERGE=4, with B3-driven demotions for `delay_differential_debt`, `hysteresis_preisach` (as a single monolithic class), `scale_free_percolation_class`, and `tail_copula_contagion` on grounds of mechanism-vs-limit-theorem confusion.

We report two qualifications plainly. First, the raw-tail Vuong likelihood ratio against lognormal is inconclusive or favors lognormal in three of nine systems (S&P 500, NIFC wildfires, Wikipedia pageviews); on log-binned BIC the same comparison flips and lognormal wins 0/7. We discuss the procedural tension in §6.2. Second, the cross-domain claim rests on the joint signature — power-law tails inside predicted bands, null controls passing, shape-normalized collapse, one non-power-law-class prediction verified (Preisach) and one pending cross-site replication (Scheffer), and taxonomy surviving multi-model critic review — and not on rejecting all smooth alternatives in every individual system.
...
3. **Universal collapse with BIC ranking.** Phase 12 polishes the cross-system collapse: log-binned density with Poisson error bars replaces the smoothed CCDF, the shape-normalized cross-system / within-system log-variance ratio reaches $r_\mathrm{shape} = 1.11$ (within the "$r < 2$" within-system FSS norm), and Bayesian model comparison prefers power-law + exponential cutoff in 5/7 systems with $\Delta\mathrm{BIC} \in [33, 967]$ — decisive — while lognormal wins 0/7. We characterise this as **consistent with a shared tail shape across seven systems**, with explicit caveats in §4.5 about row-centering and surrogate-null limitations; v0.2's stronger framing ("first positive proof of universality class membership") is demoted in v0.3 after scholar review.

4. **Two non-power-law class attempts; mixed outcome.** A2-Hysteresis confirms the Preisach first-order signature on NGSIM US-101 (median crossing time 630 s; 45 % of locations) and the loop-width ratio from two literature anchors (Treiber-Kesting A5 motorway 1.375; Geroliminis-Daganzo Yokohama 1.385; both inside $[1.25, 1.55]$). A2-Scheffer attempted the fold-bifurcation critical-slowing-down test on 14 years of Fox River DO; the naive Kendall-$\tau$ test recovered $\tau_\mathrm{AR1} = +0.284$ at $p \sim 10^{-186}$ but this p-value assumed i.i.d. observations and was numerically meaningless on a daily series with lag-1 autocorrelation 0.8-0.9. The proper moving-block-bootstrap test (l_block = 30 d, n_boot = 1,000; v4/scripts/scheffer_block_bootstrap.py) gives $p_\mathrm{block, AR1} = 0.074$, $p_\mathrm{block, Var} = 0.206$: **not significant**. We report this candidly. Scheffer-3a / -3b sub-class split (one-shot tip vs periodically-driven recurrent fold) remains a plausible structural framing but is not empirically supported by this single-site test.
...
The paper is organized as follows. Section 2 specifies the shared pipeline. Section 3 reports the case studies — nine power-law phases plus two non-power-law A2 phases. Section 4 describes universal collapse with the v0.2 log-binned-density polish and the v0.3 row-centering / surrogate-null caveats. Section 5 covers the taxonomy critic passes (B1 + B3 ensemble) and prediction calibration. Section 6 discusses the cross-domain picture honestly, including the lognormal qualification and the procedural BIC-vs-Vuong tension. **Section 7 (new in v0.3) addresses family-wise error rate / multiplicity control across the ~30 hypothesis tests.** Section 8 (new in v0.3) registers a five-system adversarial pre-registration. Section 9 (new in v0.3) makes the submission path explicit. Section 10 concludes.
...
| **7** ⚠️ | **N.A. power grid (MW load shed)** | **123** | $\mathbf{2.018 \pm 0.161}$ **([1.69, 2.31])** | **—** | $\mathbf{-0.11}$, $p = 0.92$ **(incl., small-$n$)** | **consistent w/ lit anchors; verification_independence: LOW** |
...
**Long-run secular result (v0.2 — naive Kendall $\tau$).** Over the 4,686-day analyzable interval: $\tau_\mathrm{AR1} = +0.284$ ($p_\mathrm{naive} = 1.6 \times 10^{-186}$), $\tau_\mathrm{Var} = +0.234$ ($p_\mathrm{naive} = 1.9 \times 10^{-127}$). AR1 drifts from $\sim 0.80$ in 2012 to $\sim 0.90$ in 2024 and variance rises from $\sim 0.4$ to $\sim 1.0$.

**Block-bootstrap correction (v0.3 — what the *honest* p-value looks like).** The Kendall-$\tau$ p-values quoted above assume i.i.d. observations. The Fox River daily-DO residual time series has lag-1 autocorrelation 0.8-0.9 (it *must*, to even host a Scheffer signal — that is the entire critical-slowing-down setup), which makes the i.i.d. p-value numerically meaningless. We therefore implement a moving-block bootstrap (Kunsch 1989 [61], Politis & Romano 1994 [62]) with block size $\ell_\mathrm{block} = 30$ days (rough decorrelation scale matching the rolling-mean detrend window), $n_\mathrm{boot} = 1{,}000$ resamples. For each resample, blocks of 30 contiguous days are sampled *with replacement* from the deseasoned residual series, concatenated, and the rolling 365-day AR(1) and Var are recomputed; Kendall $\tau$ is then re-evaluated against time. The block-bootstrap p-value is the fraction of $|\tau_\mathrm{boot}| \geq |\tau_\mathrm{obs}|$.

Results from `v4/scripts/scheffer_block_bootstrap.py` (seed 42, 1,000 resamples):

| Indicator | $\tau_\mathrm{obs}$ | $p_\mathrm{naive}$ | $p_\mathrm{block}$ | bootstrap mean | bootstrap p95 |
|---|---|---|---|---|---|
| Rolling AR(1) | +0.331 | $1.6 \times 10^{-245}$ | **0.074** | +0.006 | +0.305 |
| Rolling Variance | +0.239 | $2.4 \times 10^{-129}$ | **0.206** | $-$0.003 | +0.295 |

**The honest reading.** Once serial correlation is properly accounted for, the secular AR1 trend is *marginal* ($p_\mathrm{block} = 0.074$, two-sided) and the secular Var trend is *not significant* ($p_\mathrm{block} = 0.206$). The block-bootstrap bootstrap-mean is essentially zero — exactly what one would expect under a null of "random walk in a serially correlated detrended series, with no genuine secular eigenvalue drift." The original v0.2 claim of "$p \ll 10^{-120}$" was a statistical artifact of i.i.d.-assuming Kendall $\tau$ on serially correlated data, exactly the failure mode Boettiger & Hastings [60] cautioned about.

**Per-event result.** 8 of 19 events (42 %) show the naive CSD signature (simultaneously $\tau_\mathrm{AR1} > 0$ AND $\tau_\mathrm{Var} > 0$, both naive $p < 0.05$); the same serial-correlation caveat applies and we treat these as exploratory rather than confirmatory.
...
- **Shape-normalized ratio:** Each row centered ($\tilde M_{ij} = M_{ij} - \overline{M_{i \cdot}}$), absorbing the unit prefactor that $s^{*\,\alpha-1}$ does not. $r_\mathrm{shape} = 1.11$.
...
**Headline result.** Power-law + exponential cutoff wins decisively in 5/7 systems ($\Delta\mathrm{BIC} \in [33, 967]$, conventionally $> 10$ is "decisive"); pure power-law wins in 2/7 (cutoff degenerate at $s_c \to \infty$, $\Delta\mathrm{BIC} \approx 4$ — the cutoff degree of freedom is wasted); **lognormal wins in 0/7**. The seven tails share a common functional form (power-law-times-exponential-cutoff), and no two-parameter alternative competes on log-binned BIC.

**Framing (v0.3 — corrected after scholar review).** v0.1 reported "approximate collapse" as visual judgment. v0.2 framed the $r_\mathrm{shape} = 1.11$ + 7/7 lognormal rejection as "first positive proof of universality-class membership". An independent scholar reviewer (W5-A) flagged this framing as overreach, on three substantive grounds we accept:

1. **Row-centering absorbs the prefactor.** Row-centering ($\tilde M_{ij} = M_{ij} - \overline{M_{i\cdot}}$) subtracts each system's *mean log-density* across the rescaled $x'$ grid. This is mathematically equivalent to absorbing not only the dimensionful unit factor but *also* any genuine cross-system *amplitude* asymmetry. A strict cross-domain universality claim would require an *a priori* unit-consistent rescaling — e.g. all observables converted to natural-units of the underlying mechanism — and would then evaluate the cross-system variance directly, without row-centering. Our $r_\mathrm{shape}$ is therefore evidence of *shared shape after free per-system amplitude adjustment*, not of *shared shape with a constrained amplitude*. This is a meaningful but weaker statement.
2. **The "excellent $r < 2$" threshold is borrowed from within-system finite-size-scaling.** The collapse-quality threshold $r < 2$ originates in within-system finite-size studies — Pruessner [40] and Lübeck [63] use it as a within-system check: data at different system sizes should overlap on the FSS-rescaled axes, and any residual cross-size variance below 2 is read as "good FSS." Applied *cross-domain* between systems that are not even commensurate in microscopic units, the threshold has no first-principles justification. $r_\mathrm{shape} = 1.11$ is *better than naive expectation* — but how much better depends on a calibration we do not have.
3. **No surrogate-null distribution.** We did not (in this revision) compute the distribution of $r_\mathrm{shape}$ under a null hypothesis where each system's binned PDF is replaced by a draw from a different functional form (e.g. each system's actual $\alpha$ but with lognormal cutoff swapped for stretched exponential). Without such a surrogate-null, "1.11 is small" is hard to weight: 1.11 might be near-impossible under a "different shapes" null, or might be routine. See §4.5.1 below for a partial proxy.
...
- Phase 7 LR vs exponential ($p = 0.008$): does *not* survive Bonferroni at the family level, though it survives a per-class correction ($k=11$ ⇒ $\alpha_i = 0.0045$, still failing). Phase 7 is therefore not a robust exponential-rejection even before considering its low verification independence.
- Phase A2-Scheffer block-bootstrap AR1 ($p_\mathrm{block} = 0.074$): fails Bonferroni outright.
- Phase A2-Scheffer block-bootstrap Var ($p_\mathrm{block} = 0.206$): fails Bonferroni outright.

A **Holm-Bonferroni** [64] step-down procedure is more powerful at preserving FWER while not losing as much power; ordering naive p-values smallest-to-largest and accepting test $i$ only if $p_{(i)} \leq \alpha / (k - i + 1)$, the same conclusions hold: the largest-evidence tests survive (DeFi, solar exponential, wildfire LR, Wikipedia LR), the marginal tests do not.
...
| P3 | Influenza-A epidemic final-size distribution (FluNet WHO national-level, $n \approx 60$ countries × 20 years) | SIR-network contagion | $\mathbf{1.7 \pm 0.4}$ | distinct from COVID; finite-size truncation dominates; PA-like asymptote not expected |
...
- **5/5 inside band, $p_\mathrm{Bonferroni} > 0.05$ for all out-of-band counts:** the prediction framework is robust; the universality-class portability claim is strengthened to a level we currently do not report. We would update the framing accordingly.
- **4/5 inside band:** prediction framework is sound but bands need targeted recalibration on the failing class; report as "robust with one calibration update".
- **3/5 inside band:** marginal; bands are at-or-near the wrong width; report as "marginal; framework needs re-pre-registration before further claims".
- **≤ 2/5 inside band:** pipeline-level success at the functional-form level, but the predicted-band machinery is not informative; the class-membership claims in this preprint would need substantial revision.
...
The remaining qualitative results are: (i) tail-shape *consistency* (not yet *proof of universality-class membership*) has empirical content across nine power-law systems in four categories — the framing was demoted from v0.2 after we accepted the scholar reviewer's argument that row-centering absorbs system-specific amplitude prefactors and that the cross-domain $r < 2$ threshold has no first-principles justification (§4.5); (ii) the framework extends without modification to one non-power-law class prediction (Preisach) and exposed a methodologically important failure-mode in the other (Scheffer block-bootstrap); (iii) a multi-model taxonomy critic converges on demoting four classes that on inspection turn out to be mathematical frameworks rather than mechanism-defined universality classes; (iv) the §8 pre-registration on five adversarial new systems gives the next session a hard falsifiable hurdle independent of the §3 calibration set. The honest qualifications — lognormal-vs-power-law procedural ambiguity at currently available tail sizes, the row-centering trade-off, Phase 7 low verification independence, Phase A2-Scheffer block-bootstrap inconclusive, the FWER exposure across 30+ tests — are now reported in §6.5, §6.6, §7, and §9 rather than buried.
...
[61] H. R. Künsch, "The jackknife and the bootstrap for general stationary observations," *Ann. Stat.* **17**, 1217 (1989).

[62] D. N. Politis and J. P. Romano, "The stationary bootstrap," *J. Am. Stat. Assoc.* **89**, 1303 (1994).

[63] S. Lübeck, "Universal scaling behavior of non-equilibrium phase transitions," *Int. J. Mod. Phys. B* **18**, 3977 (2004).

[64] S. Holm, "A simple sequentially rejective multiple test procedure," *Scand. J. Stat.* **6**, 65 (1979).

[65] Y. Benjamini and Y. Hochberg, "Controlling the false discovery rate: a practical and powerful approach to multiple testing," *J. R. Stat. Soc. B* **57**, 289 (1995).

[66] E. Bonabeau, G. Theraulaz, and J.-L. Deneubourg, "Mathematical model of self-organization in social insect societies," *Bull. Math. Biol.* **57**, 769 (1995). [See also E. Bonabeau, *J. Theor. Biol.* **183**, 257 (1996).]

[67] D. J. Daley and J. Gani, *Epidemic Modelling: An Introduction*, Cambridge University Press (1999).
...
- **v0.3 Scheffer block-bootstrap.** Script: `v4/scripts/scheffer_block_bootstrap.py` (moving-block bootstrap, $\ell_\mathrm{block} = 30$ d, $n_\mathrm{boot} = 1{,}000$, seed 42). Output appended to `v4/validation/scheffer-lake/lake_results.json` under the `block_bootstrap` key, with both the corrected p-values and the bootstrap-distribution summary statistics.
