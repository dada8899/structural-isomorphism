...
def synthetic_series(ticker: str, dates: List[dt.date], seed: int = 0) -> List[float]:
    ...
    ***REMOVED*** annualized drift in [-0.05, 0.15], vol in [0.15, 0.45]
    mu = -0.05 + (h % 21) * 0.01
    ...
    for i, _d in enumerate(dates):
        ...
        if rally_start_idx is not None and i >= rally_start_idx:
            local_mu = mu + 0.30  ***REMOVED*** inject upward drift for "near_critical" rally pattern
...
def main(argv: List[str] | None = None) -> int:
    ...
    ***REMOVED*** For synthetic mode, extend end +12 months so backtest snapshot=today still
    ***REMOVED*** has price points covering the forward window. Real Stooq calls won't generate
    ***REMOVED*** future data (server-side cutoff) so this only affects synthetic series.
    ...
    ***REMOVED*** Sibling metadata file so downstream (backtest.py) can report data provenance honestly.
