***REMOVED*** F1 — V1/V2 Embedding Bridge

**Status**: shipped (session ***REMOVED***3, W2-E) — interface + TF-IDF fallback. Real-model wiring deferred to F1.1 (VPS deployment).
**Owner**: W2-E subagent
...
***REMOVED******REMOVED*** 2. What we're reusing
...
***REMOVED******REMOVED*** 3. Architecture
...
***REMOVED******REMOVED*** 4. API

```python
bridge = EmbeddingBridge(
    version="v2",                 ***REMOVED*** "v1" or "v2"
    fallback_mode="tfidf",        ***REMOVED*** used when model_path is missing
    model_path=None,              ***REMOVED*** auto-detects models/structural-v1
)

***REMOVED*** Suggest neighbours for a single phenomenon (dict or str)
bridge.suggest_neighbors(
    phenomenon,                   ***REMOVED*** dict with "description" / "name" / "id", or str
    k=5,
    exclude_ids=None,             ***REMOVED*** iterable[str], usually self id + known positives
) -> list[Neighbor]

***REMOVED*** Expand a candidate class: centroid of positive_examples, top-k from KB
bridge.expand_candidate_class(
    class_yaml,                   ***REMOVED*** parsed v4/taxonomy/classes/*.yaml
    k=10,
) -> list[Neighbor]

***REMOVED*** Inspection
bridge.mode            ***REMOVED*** "real_model" | "tfidf"
bridge.num_phenomena   ***REMOVED*** 4475 (v1) or 4443 (v2)
```

`Neighbor` is a dataclass with `id`, `name`, `domain`, `description`, `similarity`, and `to_dict()`.

***REMOVED******REMOVED*** 5. Where this plugs into V4 Layer 3
...
***REMOVED******REMOVED*** 6. Limitations of the TF-IDF fallback
...
***REMOVED******REMOVED*** 7. F1.1 — VPS deployment (next step, not in this PR)
...
***REMOVED******REMOVED*** 8. F2 — Active-learning loop (deferred)
...
***REMOVED******REMOVED*** 10. Decision log
