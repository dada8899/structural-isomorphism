...
def _strip_fences_and_locate(raw: str) -> str:
    ...
    s = raw.strip()
    ***REMOVED*** Strip UTF-8 BOM if present
    if s.startswith("﻿"):
        s = s.lstrip("﻿")
    ...
    ***REMOVED*** If body still has leading chatter, grab outermost JSON envelope.
...
def _replace_nan_infinity(s: str) -> str:
    ...
    """
    ***REMOVED*** Track string state and replace literal tokens at value positions.
    out = []
    i, n = 0, len(s)
    in_str = False
    str_quote = ""
    ...
    while i < n:
        ...
        if ch in ('"', "'"):
            ...
            i += 1
            continue
        ***REMOVED*** Try token match
        matched = False
        for tok, repl in tokens:
            if s.startswith(tok, i):
                ***REMOVED*** Boundary check: previous non-space char must be one of : , [
                ***REMOVED*** and next char must be terminator , } ] or whitespace.
...
def _remove_trailing_commas(s: str) -> str:
    ...
    while i < n:
        ...
        if ch in ('"', "'"):
            ...
            continue
        if ch == ",":
            ***REMOVED*** Look ahead past whitespace
            j = i + 1
            while j < n and s[j] in " \n\t\r":
                j += 1
            if j < n and s[j] in "}]":
                ***REMOVED*** skip the comma
                i += 1
                continue
        out.append(ch)
        i += 1
...
def _single_to_double_quotes(s: str) -> str:
    ...
    while i < n:
        ...
        if in_double:
            ...
            continue
        if in_single:
            ***REMOVED*** We're inside a single-quoted span we are rewriting to double-quoted.
            if ch == "\\" and i + 1 < n:
                ***REMOVED*** Preserve any escape, but if it's \', emit ' (just the char).
                if s[i + 1] == "'":
                    out.append("'")
                else:
                    out.append(ch)
            ...
            if ch == "'":
                ...
                continue
            if ch == '"':
                ***REMOVED*** Escape an inner double quote
                out.append('\\"')
                i += 1
                continue
            out.append(ch)
        ...
        ***REMOVED*** Not in any string
        ...
        out.append(ch)
        i += 1
    ***REMOVED*** Unterminated single-quoted run — close it
    if in_single:
        out.append('"')
    return "".join(out)
    ...
    while i < n:
        ...
        ***REMOVED*** In string
        if ch == "\\" and i + 1 < n:
            ...
            continue
        if ch == '"':
            ***REMOVED*** Look ahead: is this a real string-terminator?
            j = i + 1
            while j < n and s[j] in " \t\r\n":
                j += 1
            if j >= n or s[j] in ",}]:":
                ***REMOVED*** Real terminator
                out.append(ch)
                in_str = False
                i += 1
                continue
            ***REMOVED*** Unescaped interior quote — escape it
            out.append("\\")
            out.append('"')
            i += 1
            continue
