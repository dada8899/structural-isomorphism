***REMOVED*** V4 Downscoped: Transformation-Primitive Analysis on 63 A-Level Candidates

> Generated 2026-04-13. Input: V1 top-24 + V2 A-level (19) + V3 A-level (20) = 63 unique cross-domain candidates.

***REMOVED******REMOVED*** 1. Methodology

***REMOVED******REMOVED******REMOVED*** 1.1 Background
...
***REMOVED******REMOVED******REMOVED*** 1.2 Schema (8 primitives)
...
***REMOVED******REMOVED******REMOVED*** 1.3 Depth definition
...
***REMOVED******REMOVED******REMOVED*** 1.4 Labeling protocol
...
***REMOVED******REMOVED******REMOVED*** 1.5 Deduplication
...
***REMOVED******REMOVED*** 2. Primitive Distribution Across 63 Candidates
...
***REMOVED******REMOVED*** 3. Shallow vs Deep Split
...
***REMOVED******REMOVED*** 4. Top 5 Deep-Transformation Candidates

> Ranked by `transformation_depth` ≥ 2. Tie-broken by source rank/score.

***REMOVED******REMOVED******REMOVED*** ***REMOVED***1 — Earthquake Static Stress Triggering ↔ DeFi Liquidation Cascade *(V3)*
...
***REMOVED******REMOVED******REMOVED*** ***REMOVED***2 — On-chain Liquidation ↔ Earthquake Static Stress *(V3, reversed pair)*

- **Pair**: `清算级联的链上流动性危机` × `地震静态应力触发`
- **Primitives**: identical to ***REMOVED***1 (depth = 2)
- **Note**: this is the **reversed direction** of ***REMOVED***1 — V3 surfaced it as a distinct pair because the `shared_equation` is framed differently (`sum_j K_ij * delta_j > threshold_i` instead of `ΔCFS > ΔCFS_c`). For publication purposes, *****REMOVED***1 and ***REMOVED***2 collapse to a single paper**; treat this as a "both directions validated" signal rather than two independent candidates.

***REMOVED******REMOVED******REMOVED*** ***REMOVED***3 — Intersection Spillback Lock ↔ Power Grid Cascade Failure *(V3)*
...
***REMOVED******REMOVED******REMOVED*** ***REMOVED***4 — Path Integral Duality ($d \to d+1$) ↔ Neural Tangent Kernel (infinite-width limit) *(V1)*
...
***REMOVED******REMOVED******REMOVED*** ***REMOVED***5 — Amazon Rainforest Tipping ↔ Rural Depopulation Collapse *(V3)*
...
***REMOVED******REMOVED*** 5. Which Should Be Written First?

**Recommendation**: the five deep candidates rank as follows in terms of "best first arXiv paper":

1. *****REMOVED***4 Path Integral × NTK** — theoretically cleanest, uses only pencil-and-paper, suitable as a pure-theory perspective. **Lowest execution risk**.
2. *****REMOVED***1 Earthquake × DeFi** — highest surprise value, data is freely available on-chain, but requires genuine geophysics collaboration for comparison. **Highest impact if it lands**.
3. *****REMOVED***5 Rainforest × Rural Collapse** — policy-relevant, two big datasets already exist, EWS methodology is standard. **Highest societal impact**.
4. *****REMOVED***3 Traffic × Grid** — solid and quantitative but less surprising — the Motter–Lai community will likely receive it as "incremental". **Safest workshop paper**.

Candidates ***REMOVED***1 and ***REMOVED***2 should be merged into a single submission, not two.
...
***REMOVED******REMOVED*** Appendix A — Full 63 Ranked by Transformation Depth

> Format: `rank | depth | source | A ↔ B | primitives`

| ***REMOVED*** | d | src | Pair | Primitives |
|---:|---:|:---:|---|---|
...
***REMOVED******REMOVED*** Appendix B — Output files
