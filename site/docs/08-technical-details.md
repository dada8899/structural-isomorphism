...
***REMOVED******REMOVED*** 模型架构

***REMOVED******REMOVED******REMOVED*** 基座模型：text2vec-base-chinese
...
***REMOVED******REMOVED******REMOVED*** 为什么选这个模型
...
***REMOVED******REMOVED******REMOVED*** 不选大模型的原因
...
***REMOVED******REMOVED*** 训练方法

***REMOVED******REMOVED******REMOVED*** 对比学习框架
...
***REMOVED*** Data
train_examples = [
    InputExample(texts=[desc_a, desc_b])  ***REMOVED*** same type_id
...
***REMOVED******REMOVED******REMOVED*** 超参数选择依据
...
***REMOVED******REMOVED*** 数据格式详解

***REMOVED******REMOVED******REMOVED*** JSONL 文件结构

```
data/
├── taxonomy.json          ***REMOVED*** 84种数学结构的定义
├── train.jsonl            ***REMOVED*** 训练集
├── val.jsonl              ***REMOVED*** 验证集
├── test.jsonl             ***REMOVED*** 测试集
└── raw/
    └── generated.jsonl    ***REMOVED*** 原始生成数据（1260条）
```

***REMOVED******REMOVED******REMOVED*** taxonomy.json 示例
...
***REMOVED******REMOVED******REMOVED*** 训练数据示例
...
│   └── test.jsonl
├── scripts/
│   ├── generate_data.py       ***REMOVED*** 用 LLM 生成训练数据
│   ├── quality_check.py       ***REMOVED*** 数据质量检查
│   ├── prepare_pairs.py       ***REMOVED*** 构造正样本对
│   ├── train_contrastive.py   ***REMOVED*** 对比学习训练
│   ├── evaluate.py            ***REMOVED*** 评估指标计算
│   └── predict.py             ***REMOVED*** 推理脚本
├── models/                    ***REMOVED*** 保存的 checkpoint
├── checkpoints/               ***REMOVED*** 训练中间状态
├── site/                      ***REMOVED*** 项目网站（本站）
│   ├── index.html
│   ├── read.html
│   └── docs/                  ***REMOVED*** Markdown 文档
└── taxonomy-v1.md             ***REMOVED*** 分类体系文档
```

***REMOVED******REMOVED*** 评估方法

***REMOVED******REMOVED******REMOVED*** 1. Silhouette Score
...
***REMOVED*** Encode all test samples
embeddings = model.encode(test_descriptions)

***REMOVED*** Calculate silhouette score using type_id as labels
score = silhouette_score(embeddings, test_type_ids)
***REMOVED*** Target: > 0.3
```

***REMOVED******REMOVED******REMOVED*** 2. Retrieval@K
...
def retrieval_at_k(embeddings, type_ids, k=5):
    ...
    np.fill_diagonal(sim_matrix, -1)  ***REMOVED*** exclude self
...
***REMOVED******REMOVED******REMOVED*** 已知关联过滤
...
def filter_known_analogies(candidates, knowledge_base):
    ...
    for i, j, s_sim, sem_sim in candidates:
        ...
        ***REMOVED*** Skip if either lists the other as known analogy
        if name_b in known_a or name_a in known_b:
            continue

        ***REMOVED*** Skip if in same domain
...
***REMOVED******REMOVED******REMOVED*** LLM 评估 prompt
...
***REMOVED******REMOVED******REMOVED*** 阻断机制检测
