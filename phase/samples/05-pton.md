***REMOVED*** Structural Report ***REMOVED***05 — Peloton (PTON)
...
```
dS/dt = α·H(t-τ) − μ·S          ***REMOVED*** 新装硬件驱动订阅入池，τ 为交付延迟
dH/dt = β·(demand_pull − saturation)  ***REMOVED*** 疫情期间 demand_pull 被人为拉高
dC/dt = γ·peak(S, H)            ***REMOVED*** 固定成本按"最近 peak"扩张，只涨不缩
```
...
***REMOVED******REMOVED******REMOVED*** Twin ***REMOVED***1: Extinction Debt（灭绝债务，from Ecology / Tilman 1994）

**Shared equation**:
```
S(∞) = S(0) · (A_new / A_old)^z      ***REMOVED*** island biogeography, z ≈ 0.25
τ_relax ≈ 1 / (μ − α·H_steady)       ***REMOVED*** 弛豫时间：人口越接近临界越长
```
...
***REMOVED******REMOVED******REMOVED*** Twin ***REMOVED***2: Fixed Cost Collapse Under Sub-Critical Mass（from Physics / 临界质量）

**Shared equation**:
```
k_eff = (σ · N) / (L_escape + τ_absorb)      ***REMOVED*** 中子链反应
对应企业：
...
***REMOVED******REMOVED******REMOVED*** Twin ***REMOVED***3: Allee Effect in Declining Populations（from Conservation Biology）
...
***REMOVED******REMOVED*** 3. Quantified Projection（2021-01 视角，Tilman extinction debt 模型）
...
***REMOVED******REMOVED*** 4. Red Team（当时市场共识的三个结构性漏洞）
...
***REMOVED******REMOVED*** 5. Observable Early-Warning Metrics（2021-01 时点监控清单）
...
**✓ 实况验证**：指标 ***REMOVED***1 在 2021 Q4 触发（−17% YoY）、***REMOVED***7 在 2022 Q1 触发（$1.4B 库存对 $0.6B 硬件收入）、***REMOVED***2 在 2022 Q2 触发（CAC $2,100）——每一个警报后 2–6 个月，对应的崩盘阶段都真实发生了。
