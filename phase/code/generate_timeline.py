...
```json
{
  ...
  "canonical_equation": "对应方程 — **必须是合法 LaTeX**，不能含任何中文、Unicode 上下标、Unicode 希腊字母或伪代码。正确: \\\\frac{dN}{dt} = r N (1-N/K)；错误: dN/dt = r*N*(1-N/K) (饱和增长模型)",
  "key_metrics": {
    "revenue_usd_b": 5.5,
    "key_number": "MAU 2亿",
    "stock_price": "$80",
...
7. **canonical_equation 必须是合法 LaTeX**：
   - ✅ 用 `\\alpha \\beta \\theta \\partial \\nabla \\to \\approx \\sum`，不要用 `α β θ ∂ ∇ → ≈ Σ`
   - ✅ 上下标用 `^{2}` `_{i}`，不要用 Unicode `² ᵢ`
   - ✅ 分数用 `\\frac{}{}`，不要用 `/`（除非是简单商）
   - ✅ 中文注释**绝对不要**出现在方程里，要解释就单独一句话放 why_this_phase
   - ❌ 不要伪代码（如 `π(strategy_i)` `σ(θ_j)`）— 要么用合法函数名，要么别写
8. 只输出 JSON 数组，不要其他文字
"""
